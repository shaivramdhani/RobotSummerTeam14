...
#include "common/CommandTransmissionPolicy.h"
#include "common/Esp1Status.h"
#include "common/EventLog.h"
#include "common/FinalCompetitionAutonomy.h"
#include "common/FinalCompetitionStartSwitch.h"
...
constexpr std::size_t kLegacyHabitatPlacementProfileSize =
    offsetof(robot::HabitatPlacementConfig, return_line_source);
static_assert(
...
constexpr robot::Milliseconds SOLAR_LINE_REACQUIRE_STRAFE_TIMEOUT_MS =
    SOLAR_CONTACT_TIMEOUT_MS;
...
constexpr robot::SolarPanelContactConfig kSolarPanelContactConfig{
    ...
    SOLAR_LINE_REACQUIRE_STRAFE_TIMEOUT_MS};
...
static_assert(SOLAR_LINE_REACQUIRE_STRAFE_TIMEOUT_MS > 0U,
              "solar line-reacquire strafe timeout must be nonzero");
...
class RearCommandLink {
 public:
  ...
  bool send(const robot::RearDriveCommand& command) {
    ...
    const bool semantic_change =
        !last_rear_command_available_ ||
        !sameRearCommandState(command, last_rear_command_);
    if (!robot::commandTransmissionDue(
            rear_transmission_state_, semantic_change, command.enabled,
            command.sender_timestamp_ms,
            robot::commandRefreshPeriodMs(command.timeout_ms))) {
      return true;
    }
    ...
    if (sent) {
      const bool rear_line_request_changed =
          !last_rear_command_available_ ||
          command.rear_line_acquisition_enabled !=
              last_rear_command_.rear_line_acquisition_enabled;
      if (rear_line_request_changed) {
        rear_line_acquisition_requested_ =
            command.rear_line_acquisition_enabled;
        rear_line_snapshot_received_for_request_ = false;
        rear_line_request_started_at_ms_ = command.sender_timestamp_ms;
      }
      last_rear_command_ = command;
      last_rear_command_available_ = true;
      robot::noteCommandTransmitted(rear_transmission_state_,
                                    semantic_change,
                                    command.sender_timestamp_ms);
  ...
  bool send(const robot::FunnelCommand& command) {
    ...
    const bool semantic_change =
        !last_funnel_command_available_ ||
        !sameFunnelCommandState(command, last_funnel_command_);
    if (!robot::commandTransmissionDue(
            funnel_transmission_state_, semantic_change, command.enabled,
            command.sender_timestamp_ms,
            robot::commandRefreshPeriodMs(command.timeout_ms))) {
      return true;
    }
    ...
    const bool sent = sendPacket(packet);
    if (sent) {
      last_funnel_command_ = command;
      last_funnel_command_available_ = true;
      robot::noteCommandTransmitted(funnel_transmission_state_,
                                    semantic_change,
                                    command.sender_timestamp_ms);
    }
    return sent;
  ...
  bool send(const robot::SolarHookCommand& command) {
    ...
    const bool semantic_change =
        !last_solar_hook_command_available_ ||
        !sameSolarHookCommandState(command, last_solar_hook_command_);
    if (!robot::commandTransmissionDue(
            solar_hook_transmission_state_, semantic_change, false, now_ms,
            robot::commandRefreshPeriodMs(
    ...
    const bool sent = sendPacket(packet);
    if (sent) {
      last_solar_hook_command_ = command;
      last_solar_hook_command_available_ = true;
      robot::noteCommandTransmitted(solar_hook_transmission_state_,
                                    semantic_change, now_ms);
    }
    return sent;
  }

 private:
  static bool sameRearCommandState(
      const robot::RearDriveCommand& lhs,
      const robot::RearDriveCommand& rhs) {
    return lhs.enabled == rhs.enabled &&
           lhs.back_left_command_milli == rhs.back_left_command_milli &&
           lhs.back_right_command_milli == rhs.back_right_command_milli &&
           lhs.timeout_ms == rhs.timeout_ms &&
           lhs.laser_acquisition_enabled == rhs.laser_acquisition_enabled &&
           lhs.ir_acquisition_enabled == rhs.ir_acquisition_enabled &&
           lhs.rear_line_acquisition_enabled ==
               rhs.rear_line_acquisition_enabled &&
           lhs.laser_profile == rhs.laser_profile;
  }

  static bool sameFunnelCommandState(
      const robot::FunnelCommand& lhs,
      const robot::FunnelCommand& rhs) {
    return lhs.enabled == rhs.enabled &&
           lhs.command_milli == rhs.command_milli &&
           lhs.timeout_ms == rhs.timeout_ms;
  }

  static bool sameSolarHookCommandState(
      const robot::SolarHookCommand& lhs,
      const robot::SolarHookCommand& rhs) {
    return lhs.enabled == rhs.enabled && lhs.angle_deg == rhs.angle_deg;
  }
 ...
 public:
    ...
    while (Serial1.available() > 0) {
      ...
      if (status == robot::UartFrameParserStatus::PacketReady) {
        if (packet.header.message_type ==
          ...
          robot::Esp1StatusReport report = latest_status_;
          if (robot::decodeEsp1StatusPacket(packet, report)) {
            ...
            ++packet_error_count_;
                   ...
                   robot::UartMessageType::DiagnosticReport) {
          robot::Esp1StatusReport report = latest_status_;
          if (robot::decodeEsp1DiagnosticsPacket(packet, report)) {
            latest_status_ = report;
          } else {
            ++packet_error_count_;
          }
          ...
          if (robot::decodeRearLineSensorPacket(packet, snapshot)) {
            if (rear_line_snapshot_available_ &&
              ...
              if (rear_line_acquisition_requested_) {
                rear_line_snapshot_received_for_request_ = true;
              }
            }
          } else {
            ++packet_error_count_;
  ...
  bool rearLineAcquisitionRequested() const {
    return rear_line_acquisition_requested_;
  }
  bool rearLineStreamWarming() const {
    return rear_line_acquisition_requested_ &&
           !rear_line_snapshot_received_for_request_;
  }
  robot::Milliseconds rearLineRequestStartedAtMs() const {
    return rear_line_request_started_at_ms_;
  }
 ...
 private:
  ...
  robot::CommandTransmissionState rear_transmission_state_{};
  robot::CommandTransmissionState funnel_transmission_state_{};
  robot::CommandTransmissionState solar_hook_transmission_state_{};
  robot::RearDriveCommand last_rear_command_{};
  robot::FunnelCommand last_funnel_command_{};
  robot::SolarHookCommand last_solar_hook_command_{};
  bool last_rear_command_available_{false};
  bool last_funnel_command_available_{false};
  bool last_solar_hook_command_available_{false};
  bool rear_line_acquisition_requested_{false};
  bool rear_line_snapshot_received_for_request_{false};
  robot::Milliseconds rear_line_request_started_at_ms_{0U};
...
class ClawServoBank {
 public:
  ...
  bool allClawTargetsConfigured() const {
    for (std::size_t index = 0U; index < kClawServoCount; ++index) {
      int target_angle_deg = kClawServoUnsetAngleDeg;
      if (targetAngle(index, ClawServoPositionRequest::Open,
                      target_angle_deg) !=
  ...
  ClawServoCommandResult commandExclusiveOpenClaw(
      const std::size_t open_claw_index) {
    if (open_claw_index >= kClawServoCount) {
    ...
    for (std::size_t index = 0U; index < kClawServoCount; ++index) {
      const ClawServoPositionRequest request =
          index == open_claw_index ? ClawServoPositionRequest::Open
                                   : ClawServoPositionRequest::Closed;
      const ClawServoCommandResult result =
    ...
    for (std::size_t index = 0U; index < kClawServoCount; ++index) {
      ...
      commanded_open_[index] = index == open_claw_index;
    }
    return ClawServoCommandResult::Accepted;
  }
...
struct RuntimeContext {
  ...
  HabitatPiecesProfiles habitat_pieces_profiles{};
  std::uint8_t habitat_pieces_editor_profile_index{0U};
  ...
  std::int64_t habitat_pieces_lift_target_steps{0};
  HabitatPlacementProfiles habitat_placement_profiles{};
  std::uint8_t habitat_placement_editor_profile_index{0U};
  ...
  robot::HabitatCycleAutonomy habitat_cycle{};
  ...
  robot::Milliseconds solar_funnel_open_started_at_ms{0U};
  ...
  bool solar_completion_mechanisms_started{false};
  bool solar_funnel_open_complete{false};
  ...
  bool solar_hook_hold_open_for_mission{false};
...
struct RuntimeBindings {
  ...
  DigitalActiveHighLimitSwitch* habitat_piece_limit{nullptr};
...
bool refreshSharedImuSnapshot(RuntimeContext& context) {
  if (g_runtime.imu_acquisition == nullptr) {
    context.imu_shared_snapshot_available = false;
    context.imu_shared_snapshot_fetched_at_us = micros();
    return false;
  }

  context.imu_shared_snapshot_available =
      g_runtime.imu_acquisition->latest(context.latest_imu_snapshot);
  context.imu_shared_snapshot_fetched_at_us = micros();
  ...
  return context.imu_shared_snapshot_available;
}
...
bool rearLineAcquisitionRequested(const RuntimeContext& context) {
  switch (context.modes.currentMode()) {
    case robot::RobotTestMode::RearLineSensorTest:
    case robot::RobotTestMode::RearLineFollowTest:
    case robot::RobotTestMode::HabitatPieces:
    case robot::RobotTestMode::HabitatPlacement:
    case robot::RobotTestMode::AutonomousTowerPieces:
      return true;
    case robot::RobotTestMode::TimeTrial:
      return context.time_trial.state ==
                 robot::TimeTrialState::WaitForStart ||
             context.time_trial.state ==
    ...
    case robot::RobotTestMode::FinalCompetition:
      return context.final_competition.state ==
...
robot::LineFollowerConfig habitatPiecesLineFollowerConfig(
    ...
    const robot::HabitatPiecesConfig& habitat_config) {
  robot::LineFollowerConfig config = context.config;
  config.baseDuty = habitat_config.line_follow_duty;
  return config;
}

robot::LineFollowerConfig habitatPiecesLineFollowerConfig(
    const RuntimeContext& context) {
  return habitatPiecesLineFollowerConfig(
      context, context.habitat_pieces_config);
}
...
robot::LineFollowerConfig habitatPlacementLineFollowerConfig(
    const RuntimeContext& context,
    const robot::HabitatPlacementConfig& placement_config) {
  return towerPiecesLineFollowerConfig(
      context, placement_config.reverse_line_follow_duty);
}

robot::LineFollowerConfig habitatPlacementLineFollowerConfig(
    const RuntimeContext& context) {
  return habitatPlacementLineFollowerConfig(
      context, context.habitat_placement_config);
}
...
void resetSolarPanelAutonomy(RuntimeContext& context,
  ...
  context.solar_funnel_open_started_at_ms = 0U;
  context.solar_completion_mechanisms_started = false;
  context.solar_funnel_open_complete = false;
...
void resetHabitatPieces(RuntimeContext& context,
  ...
  context.habitat_pieces_lift_target_steps = 0;
  context.habitat_pieces_start_requested = false;
  if (context.habitat_pieces_editor_profile_index >=
      robot::kHabitatProfileCount) {
    context.habitat_pieces_editor_profile_index = 0U;
  }
  context.habitat_pieces_config =
      context.habitat_pieces_profiles[
          context.habitat_pieces_editor_profile_index];
  robot::resetImuHeadingHoldController(context.imu_heading_hold_state);
  context.last_imu_heading_hold_update = {};
  robot::cancelImuRecovery(context.imu_strafe_recovery);
}
...
void resetHabitatPlacement(RuntimeContext& context,
  ...
  if (context.habitat_placement_editor_profile_index >=
      robot::kHabitatProfileCount) {
    context.habitat_placement_editor_profile_index = 0U;
  }
  context.habitat_placement_config =
      context.habitat_placement_profiles[
          context.habitat_placement_editor_profile_index];
...
robot::FourWheelCommand makeHabitatPiecesTranslationCommand(
  ...
  return robot::mixOpenLoopMecanum(
      vx, vy, 0.0F,
      clampFloat(duty, 0.0F, activeMotionDutyCap(context)), now_ms,
      context.config.remoteCommandTimeoutMs);
}

robot::FourWheelCommand makeHabitatPiecesBackwardCommand(
    const RuntimeContext& context, const robot::Milliseconds now_ms) {
  return makeHabitatPiecesTranslationCommand(
      context, 0.0F, -1.0F,
...
bool sendStoppedRearCommand(RearCommandLink& rear_link,
                            ...
                            const bool rear_line_acquisition_enabled = false) {
...
enum class SolarCompletionMechanismResult : std::uint8_t {
  Running = 0,
  Complete = 1,
  HardwareUnavailable = 2,
  SolarHookCommandFailed = 3,
  FunnelCommandFailed = 4,
};

SolarCompletionMechanismResult startSolarFunnelOpening(
    RuntimeContext& context, RearCommandLink& rear_link,
    const robot::Milliseconds now_ms) {
  if (context.solar_completion_mechanisms_started) {
    return context.solar_funnel_open_complete
               ? SolarCompletionMechanismResult::Complete
               : SolarCompletionMechanismResult::Running;
  }
  if (!rear_link.remoteStatusFresh(
    ...
    return SolarCompletionMechanismResult::HardwareUnavailable;
  }

  context.solar_funnel_open_started_at_ms = now_ms;
  context.solar_completion_mechanisms_started = true;
  context.solar_funnel_open_complete = false;
  ...
  if (!sendFunnelMotorCommand(rear_link,
    ...
    context.solar_completion_mechanisms_started = false;
    return SolarCompletionMechanismResult::FunnelCommandFailed;
  }
  context.last_command_ms = now_ms;
  return SolarCompletionMechanismResult::Running;
}

SolarCompletionMechanismResult openSolarHookForMission(
    RuntimeContext& context, RearCommandLink& rear_link,
    const robot::Milliseconds now_ms) {
  if (!rear_link.remoteStatusFresh(
          now_ms, remoteStatusTimeoutMs(context.config)) ||
  ...
  if (!sendSolarHookPosition(context, rear_link, true)) {
    return SolarCompletionMechanismResult::SolarHookCommandFailed;
  }
  context.solar_hook_hold_open_for_mission = true;
  return SolarCompletionMechanismResult::Complete;
}

SolarCompletionMechanismResult serviceSolarCompletionMechanisms(
    RuntimeContext& context, RearCommandLink& rear_link,
    const robot::Milliseconds now_ms) {
  if (!context.solar_completion_mechanisms_started) {
    return SolarCompletionMechanismResult::HardwareUnavailable;
  }
  if (context.solar_funnel_open_complete) {
    return SolarCompletionMechanismResult::Complete;
  }
  if (!rear_link.remoteStatusFresh(
          now_ms, remoteStatusTimeoutMs(context.config)) ||
  ...
  const robot::Milliseconds funnel_elapsed_ms = elapsedSince(
      now_ms, context.solar_funnel_open_started_at_ms);
  if (funnel_elapsed_ms >=
      context.solar_contact_config.funnel_open_duration_ms) {
    context.requested_funnel_command = robot::disabledMotorCommand();
    if (!sendStoppedFunnelCommand(rear_link, context.config, now_ms)) {
      return SolarCompletionMechanismResult::FunnelCommandFailed;
    }
    context.solar_funnel_open_complete = true;
    return SolarCompletionMechanismResult::Complete;
  }
...
bool disableMotionActuators(RuntimeContext& context,
  ...
  return sendStoppedRearCommand(rear_link, context.config, now_ms,
                                ...
                                rearLineAcquisitionRequested(context));
}
...
void disableActuators(RuntimeContext& context, robot::IMotorOutput& front_left,
  ...
  context.solar_hook_commanded_open = false;
  context.solar_hook_hold_open_for_mission = false;
  if (g_runtime.claws != nullptr) {
    g_runtime.claws->disable();
  }
}

void cancelHabitatCycle(RuntimeContext& context) {
  robot::resetHabitatCycleAutonomy(context.habitat_cycle);
}
...
bool habitatPiecesProfileConfigValid(
    const RuntimeContext& context,
    const robot::HabitatPiecesConfig& config,
    const robot::esp2::StepperAxis& stepper) {
  return robot::habitatPiecesConfigValid(
             config, activeMotionDutyCap(context),
             kMaxTimedTestDurationMs) &&
         robot::validateLineFollowerConfig(
             habitatPiecesLineFollowerConfig(context, config),
             hardwareDutyCap())
             .accepted &&
         imuHeadingHoldRuntimeConfigValid(
             context.imu_heading_hold_config) &&
         autonomousStrafeDutyValid(
             context, config.distance_strafe_duty) &&
         autonomousStrafeDutyValid(
             context, config.exit_strafe_duty) &&
         autonomousStrafeDutyValid(
             context, config.rear_line_reacquire_duty) &&
         config.lift_steps <= static_cast<std::uint64_t>(
...
bool allHabitatPiecesProfilesValid(
    const RuntimeContext& context,
    const robot::esp2::StepperAxis& stepper) {
  for (const robot::HabitatPiecesConfig& config :
       context.habitat_pieces_profiles) {
    if (!habitatPiecesProfileConfigValid(context, config, stepper)) {
      return false;
    }
  }
  return true;
...
bool habitatPiecesMotionRequirementsMet(
  ...
  return startRequirementsMet(sensors, front_left, front_right, rear_link,
                              now_ms, context) &&
         habitat_piece_limit.configured() &&
         ...
         habitatPiecesProfileConfigValid(
             context, context.habitat_pieces_config, stepper);
}
...
bool habitatPlacementImuStrafeConfigValid(
    const RuntimeContext& context,
    const robot::HabitatPlacementConfig& config) {
  return autonomousStrafeDutyValid(
             context,
             config.pre_counter_clockwise_strafe_right_duty) &&
         autonomousStrafeDutyValid(
             context, config.post_clockwise_strafe_left_duty) &&
         autonomousStrafeDutyValid(context, config.strafe_right_duty);
}

bool habitatPlacementProfileConfigValid(
    const RuntimeContext& context,
    const robot::HabitatPlacementConfig& config,
    const robot::esp2::StepperAxis& stepper) {
  ...
  return bounded_timings &&
         habitatPlacementImuStrafeConfigValid(context, config) &&
         imuTurnRuntimeConfigValid(initial_heading_turn_config) &&
         imuTurnRuntimeConfigValid(ccw_turn_config) &&
         imuTurnRuntimeConfigValid(cw_turn_config) &&
         robot::validateLineFollowerConfig(
             habitatPlacementLineFollowerConfig(context, config),
             hardwareDutyCap())
             .accepted &&
...
bool allHabitatPlacementProfilesValid(
    const RuntimeContext& context,
    const robot::esp2::StepperAxis& stepper) {
  for (const robot::HabitatPlacementConfig& config :
       context.habitat_placement_profiles) {
    if (!habitatPlacementProfileConfigValid(context, config, stepper)) {
      return false;
    }
  }
  return true;
...
bool habitatPlacementStartRequirementsMet(
    ...
    const robot::Milliseconds now_ms,
    const RuntimeContext& context) {
  const bool return_sensor_ready =
      context.habitat_placement_config.return_line_source !=
          robot::HabitatPlacementReturnLineSource::Front ||
      sensors.configured();
  return return_sensor_ready &&
         ...
         !(stepper.lowerLimitActive() && stepper.upperLimitActive()) &&
         imuReadyForAutonomousMotion(context) &&
         habitatPlacementProfileConfigValid(
             context, context.habitat_placement_config, stepper);
}

bool habitatSequenceStartRequirementsMet(
    const DigitalFrontLineSensorReader& sensors,
    const DualPwmMotorOutput& front_left,
    const DualPwmMotorOutput& front_right,
    const RearCommandLink& rear_link,
    ...
    const robot::Milliseconds now_ms,
    const RuntimeContext& context) {
  bool any_placement_uses_front_line = false;
  for (const robot::HabitatPlacementConfig& config :
       context.habitat_placement_profiles) {
    any_placement_uses_front_line =
        any_placement_uses_front_line ||
        config.return_line_source ==
            robot::HabitatPlacementReturnLineSource::Front;
  }
  return allHabitatPiecesProfilesValid(context, stepper) &&
         allHabitatPlacementProfilesValid(context, stepper) &&
         (!any_placement_uses_front_line || sensors.configured()) &&
         habitatPiecesStartRequirementsMet(
             sensors, front_left, front_right, rear_link,
             habitat_piece_limit, claws, stepper, now_ms, context) &&
         habitatPlacementStartRequirementsMet(
             sensors, front_left, front_right, rear_link, claws,
             stepper, now_ms, context);
}
...
bool pegFinderStartRequirementsMet(
  ...
  return front_left.configured() && front_right.configured() &&
         ...
         claws.allClawTargetsConfigured() &&
...
bool solarPanelStartRequirementsMet(
  ...
  return startRequirementsMet(sensors, front_left, front_right, rear_link,
         ...
         context.solar_contact_config.funnel_open_duration_ms <=
             kMaxTimedTestDurationMs &&
         context.solar_contact_config.line_reacquire_strafe_timeout_ms <=
             kMaxTimedTestDurationMs;
}
...
bool sendRearWheelCommand(RearCommandLink& rear_link,
                          ...
                          const bool rear_line_acquisition_enabled) {
  ...
  rear.rear_line_acquisition_enabled =
      rear_line_acquisition_enabled;
...
AutonomousImuRecoveryUpdate serviceAutonomousImuRecovery(
  ...
  // WebServer callbacks and other core-1 work run between the loop's initial
  // snapshot copy and autonomous motion. Re-peek the producer queue at the
  // safety gate so a healthy current sample cannot be rejected merely because
  // the earlier cached copy aged while that work completed.
  (void)refreshSharedImuSnapshot(context);
  const robot::esp2::ImuState& imu_state =
      context.latest_imu_snapshot.state;
...
AutonomousImuMotionResult runAutonomousImuStrafe(
    ...
    const robot::Milliseconds now_ms,
    const bool refresh_and_validate_imu = true) {
  // Habitat passes false only after serviceAutonomousImuRecovery validated the
  // cached snapshot in the same phase invocation. Reusing that exact snapshot
  // keeps recovery and control on one coherent availability decision.
  if (refresh_and_validate_imu) {
    (void)refreshSharedImuSnapshot(context);
  }
...
AutonomousImuMotionResult runAutonomousImuTurnImpl(
    ...
    const bool refresh_and_validate_imu) {
  if (refresh_and_validate_imu) {
    (void)refreshSharedImuSnapshot(context);
  }
...
void requestHabitatPiecesStart(RuntimeContext& context,
  ...
  if (final_competition_parent ||
      context.solar_hook_hold_open_for_mission) {
    // The raised solar hook is needed by the later Habitat/Tower route. Stop
    // chassis and funnel outputs without detaching the hook servo PWM.
  ...
  resetPegFinder(context, now_ms);
  resetTimeTrial(context, now_ms);
  if (context.habitat_cycle.phase == robot::HabitatCyclePhase::Pickup &&
      context.habitat_cycle.active_profile_index <
          robot::kHabitatProfileCount) {
    context.habitat_pieces_config =
        context.habitat_pieces_profiles[
            context.habitat_cycle.active_profile_index];
...
void serviceFinalCompetitionStartSwitch(
    RuntimeContext& context,
    const DigitalActiveHighLimitSwitch& start_switch,
    robot::IMotorOutput& front_left, robot::IMotorOutput& front_right,
    RearCommandLink& rear_link, const robot::Milliseconds now_ms) {
  if (!start_switch.configured()) {
    return;
  }
  ...
  const bool final_competition_mode_active =
      context.modes.currentMode() ==
      robot::RobotTestMode::FinalCompetition;
  const robot::FinalCompetitionStartSwitchAction action =
      robot::updateFinalCompetitionStartSwitch(
          context.final_competition_start_switch, start_switch.active(),
          final_competition_mode_active);
  switch (action) {
    case robot::FinalCompetitionStartSwitchAction::Start:
      requestFinalCompetitionStart(
          context, front_left, front_right, rear_link, now_ms,
          robot::EventSource::System);
      return;
    case robot::FinalCompetitionStartSwitchAction::EmergencyStop:
      logEvent(context, now_ms, robot::EventSeverity::Warn,
               robot::EventSource::System,
               "Final competition stopped: GPIO10 switch LOW");
      emergencyStop(context, front_left, front_right, rear_link, now_ms,
                    robot::EventSource::System);
      return;
    case robot::FinalCompetitionStartSwitchAction::None:
      return;
  }
}
...
void enterHabitatPiecesFault(
  ...
  context.last_habitat_pieces_update.should_wait_after_distance_count =
      false;
  context.last_habitat_pieces_update.should_exit_strafe_pulse = false;
  context.last_habitat_pieces_update.should_check_exit_distance = false;
  context.last_habitat_pieces_update.should_lower_slide = false;
  context.last_habitat_pieces_update.should_drive_forward_to_piece = false;
  context.last_habitat_pieces_update.should_drive_back_before_lift = false;
  context.last_habitat_pieces_update.should_start_lift = false;
  context.last_habitat_pieces_update.should_drive_back_after_pickup = false;
  context.last_habitat_pieces_update.should_reacquire_rear_line = false;
  context.last_habitat_pieces_update.should_wait_for_lift = false;
  context.last_habitat_pieces_update.should_start_habitat_placement = false;
  robot::failHabitatCycleAutonomy(context.habitat_cycle);
  setFault(context, fault_code, message);
  logEvent(context, now_ms, robot::EventSeverity::Fault, source, message);
}

void enterHabitatPiecesImuStrafeFault(
    RuntimeContext& context, DualPwmMotorOutput& front_left,
    DualPwmMotorOutput& front_right, RearCommandLink& rear_link,
  ...
  enterHabitatPiecesFault(
      context, front_left, front_right, rear_link,
      imu_unavailable
          ? robot::HabitatPiecesStopReason::ImuUnavailable
          : (link_failed
                 ? robot::HabitatPiecesStopReason::RearCommandFailed
                 : robot::HabitatPiecesStopReason::ImuStrafeFailed),
      imu_unavailable ||
          ...
          : (link_failed ? robot::FaultCode::CommunicationStale
                         : robot::FaultCode::InvalidCommand),
      imu_unavailable
          ? "Habitat Pieces stopped: IMU unavailable"
          : (link_failed
                 ? "Habitat Pieces stopped: IMU strafe command failed"
                 : "Habitat Pieces stopped: IMU heading-hold strafe failed"),
      link_failed ? robot::EventSource::Uart
                  : robot::EventSource::System,
      now_ms);
}

AutonomousImuRecoveryUpdate serviceHabitatPiecesImuRecovery(
    RuntimeContext& context, DualPwmMotorOutput& front_left,
    DualPwmMotorOutput& front_right, RearCommandLink& rear_link,
    const robot::Milliseconds now_ms) {
  ...
  context.habitat_pieces.state_entered_at_ms +=
      recovery_update.timer_adjustment_ms;
  context.habitat_pieces.distance_strafe_started_at_ms +=
      recovery_update.timer_adjustment_ms;
  if (context.habitat_pieces.lift_started_at_ms != 0U) {
    context.habitat_pieces.lift_started_at_ms +=
        recovery_update.timer_adjustment_ms;
  }
...
void runHabitatPieces(
  ...
  if (context.habitat_pieces.state ==
    ...
    const bool start_ready =
        robot::habitatCycleActive(context.habitat_cycle)
            ...
            : habitatPiecesStartRequirementsMet(
                  sensors, front_left, front_right, rear_link,
                  habitat_piece_limit, claws, stepper, now_ms,
                  context);
    if (!start_ready) {
      enterHabitatPiecesFault(
          context, front_left, front_right, rear_link,
          ...
          robot::FaultCode::HardwareNotConfigured,
          "Habitat sequence start rejected: one or more pickup/placement profiles or hardware are incomplete",
          robot::EventSource::System, now_ms);
      return;
    }
  ...
  if (robot::habitatPiecesStateRequiresImuStrafe(
          context.habitat_pieces.state)) {
    const AutonomousImuRecoveryUpdate recovery_update =
        serviceHabitatPiecesImuRecovery(
            context, front_left, front_right, rear_link, now_ms);
    ...
    if (recovery_update.result ==
        ...
        recovery_update.result ==
            AutonomousImuRecoveryResult::StopCommandFailed) {
      enterHabitatPiecesImuStrafeFault(
          context, front_left, front_right, rear_link,
          recovery_update.result ==
                  AutonomousImuRecoveryResult::TimedOut
  ...
  if (context.habitat_pieces.state ==
    ...
    lss3_black = line_sensors.side_3_configured &&
                 line_sensors.side_3_electrical_high;
  }
  ...
  if (rear_link.laserSnapshotAvailable()) {
    ...
    const robot::Milliseconds maximum_sample_age_ms =
        ...
        4U;
    const bool fresh_sensor_attempt =
        laser.configured && laser.initialized && laser.ranging &&
        laser.profile == robot::kOperationalLaserDistanceProfile &&
    ...
    const bool valid_target =
        fresh_sensor_attempt && laser.data_valid &&
        laser.sensor_range_status == robot::kVl53l0xValidRangeStatus &&
        laser.driver_status == 0;
    distance_sample.available = fresh_sensor_attempt;
    distance_sample.distance_mm =
        valid_target
            ? static_cast<std::uint32_t>(laser.distance_mm)
    ...
    distance_sample.substituted_no_target =
        fresh_sensor_attempt && !valid_target;
  }
  ...
  if (stepper.lowerLimitActive() && stepper.upperLimitActive()) {
    enterHabitatPiecesFault(
        ...
        "Habitat Pieces stopped: stepper limits conflict",
        robot::EventSource::Motor, now_ms);
    return;
  }
  ...
  const bool lift_state =
      ...
      previous_state == robot::HabitatPiecesState::RearLineReacquire ||
      previous_state ==
          robot::HabitatPiecesState::WaitForLiftCompletion;
  const bool lift_complete =
      context.habitat_pieces_lift_target_steps > 0 &&
  ...
  if (context.habitat_pieces.slide_down_started &&
    ...
    enterHabitatPiecesFault(
        ...
        "Habitat Pieces stopped: slide stopped before bottom limit",
        robot::EventSource::Motor, now_ms);
    return;
  }
  if (lift_state && !lift_complete &&
      (stepper.motionState() ==
           robot::esp2::StepperMotionState::LimitSearchFailed ||
       stepper.motionState() ==
           robot::esp2::StepperMotionState::Stopped)) {
    enterHabitatPiecesFault(
        ...
        robot::HabitatPiecesStopReason::StepperCommandFailed,
        robot::FaultCode::InvalidCommand,
        "Habitat Pieces stopped: slide lift stopped before target",
        robot::EventSource::Motor, now_ms);
    return;
  }

  const bool rear_line_fresh =
      rear_link.rearLineSnapshotFresh(
          now_ms, context.rear_config.remoteCommandTimeoutMs);
  const robot::RearLineSensorSnapshot& rear_line =
      rear_link.latestRearLineSnapshot();
  if (previous_state == robot::HabitatPiecesState::RearLineReacquire &&
      (!rear_line_fresh || !rear_line.configured)) {
    enterHabitatPiecesFault(
        context, front_left, front_right, rear_link,
        !rear_line_fresh
            ? robot::HabitatPiecesStopReason::SideSensorDataStale
            : robot::HabitatPiecesStopReason::SideSensorsUnavailable,
        !rear_line_fresh ? robot::FaultCode::CommunicationStale
                         : robot::FaultCode::HardwareNotConfigured,
        !rear_line_fresh
            ? "Habitat Pieces stopped: rear line data stale"
            : "Habitat Pieces stopped: rear line sensors unavailable",
        !rear_line_fresh ? robot::EventSource::Uart
                         : robot::EventSource::Line,
        now_ms);
    return;
  }

  robot::HabitatPiecesMechanismInputs mechanism_inputs{};
  mechanism_inputs.bottom_limit_active = stepper.lowerLimitActive();
  mechanism_inputs.approach_limit_active = habitat_piece_limit.active();
  mechanism_inputs.lift_complete = lift_complete;
  mechanism_inputs.pusher_closed_commanded =
      claws.habitatPusherCommandedClosed();
  mechanism_inputs.rear_line_available =
      rear_line_fresh && rear_line.configured;
  ...
  if (habitat_update.state != previous_state) {
    ...
    const bool distance_timeout_fallback =
        habitat_update.stop_reason ==
            robot::HabitatPiecesStopReason::DistanceStrafeTimeout &&
        (previous_state == robot::HabitatPiecesState::DistanceStrafing ||
         previous_state == robot::HabitatPiecesState::PostCountStopDelay ||
         previous_state == robot::HabitatPiecesState::ExitStrafePulse ||
         previous_state == robot::HabitatPiecesState::ExitDistanceCheck);
    if (distance_timeout_fallback) {
      const bool counting_strafe_expired =
          previous_state == robot::HabitatPiecesState::DistanceStrafing;
      logEvent(
          context, now_ms, robot::EventSeverity::Warn,
          robot::EventSource::System,
          counting_strafe_expired
              ? "Habitat Pieces distance count strafe timed out; continuing post-count exit sequence"
              : "Habitat Pieces distance exit sequence timed out; continuing pickup tail");
    }
    if (previous_state == robot::HabitatPiecesState::ApproachPiece &&
        habitat_update.state ==
      ...
      logEvent(
          context, now_ms, robot::EventSeverity::Warn,
          robot::EventSource::System,
          "Habitat Pieces approach timed out; continuing pickup lift/reverse fallback");
    }

    // The state transition happens after the normal per-phase recovery gate.
    // Gate the first iteration here as well so a temporary IMU outage pauses
    // the newly entered strafe instead of becoming a terminal route fault.
    if (robot::habitatPiecesStateRequiresImuStrafe(
            habitat_update.state)) {
      const AutonomousImuRecoveryUpdate recovery_update =
          serviceHabitatPiecesImuRecovery(
  ...
  if (habitat_update.should_start_lift) {
    context.habitat_pieces_lift_target_steps =
        stepper.positionSteps() +
        static_cast<std::int64_t>(
            context.habitat_pieces_config.lift_steps);
    const bool accepted =
        stepper.setSpeed(
            context.habitat_pieces_config
                .lift_speed_steps_per_second) &&
  ...
  if (habitat_update.should_close_pusher) {
    robot::stopLineFollower(context.follower_state);
    stopAutonomousImuStrafe(context);
    if (!disableMotionActuators(context, front_left, front_right,
                                rear_link, now_ms)) {
      enterHabitatPiecesFault(
          context, front_left, front_right, rear_link,
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          "Habitat Pieces stopped: wheel stop failed before pusher close",
          robot::EventSource::Uart, now_ms);
      return;
    }
    const ClawServoCommandResult pusher_result = claws.command(
        kHabitatPusherServoIndex, ClawServoPositionRequest::Closed);
    if (pusher_result != ClawServoCommandResult::Accepted) {
      enterHabitatPiecesFault(
          context, front_left, front_right, rear_link,
          robot::HabitatPiecesStopReason::PusherCommandFailed,
          clawFaultCode(pusher_result),
          "Habitat Pieces stopped: pusher close command failed",
          robot::EventSource::Motor, now_ms);
      return;
    }
    if (habitat_update.transitioned) {
      logEvent(context, now_ms, robot::EventSeverity::Info,
               robot::EventSource::Motor,
               "Habitat Pieces reverse complete; pusher close commanded before distance strafe");
    }
    return;
  }
  ...
  if (context.last_habitat_pieces_update.should_distance_strafe ||
      context.last_habitat_pieces_update.should_exit_strafe_pulse) {
    robot::stopLineFollower(context.follower_state);
    if (context.last_habitat_pieces_update.transitioned) {
      logEvent(
          ...
          robot::EventSource::System,
          context.last_habitat_pieces_update.should_exit_strafe_pulse
              ? "Habitat Pieces distance-exit strafe pulse started"
              : "Habitat Pieces pusher close confirmed; distance-zone strafe started");
    }
    const AutonomousImuMotionResult result =
        runAutonomousImuStrafe(
            context, front_left, front_right, rear_link, context.config,
            context.habitat_pieces_config.distance_strafe_direction ==
                    robot::HabitatPiecesStrafeDirection::Right
                ? 1
                : -1,
            context.last_habitat_pieces_update.should_exit_strafe_pulse
                ? context.habitat_pieces_config.exit_strafe_duty
                : context.habitat_pieces_config.distance_strafe_duty,
            now_ms, false);
    if (result != AutonomousImuMotionResult::Running) {
      enterHabitatPiecesImuStrafeFault(
          context, front_left, front_right, rear_link, result, now_ms);
      return;
    }
    context.last_command_ms = now_ms;
    printTelemetry(context, sensors, rear_link, now_ms);
  ...
  if (context.last_habitat_pieces_update
          .should_wait_after_distance_count ||
      context.last_habitat_pieces_update.should_check_exit_distance) {
    robot::stopLineFollower(context.follower_state);
    if (!disableMotionActuators(context, front_left, front_right,
                                rear_link, now_ms)) {
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          "Habitat Pieces stopped: distance-check stop command failed",
          robot::EventSource::Uart, now_ms);
      return;
    }
    if (context.last_habitat_pieces_update.transitioned) {
      logEvent(
          ...
          context.last_habitat_pieces_update
                  .should_wait_after_distance_count
              ? "Habitat Pieces distance-zone target reached; stop delay started"
              : "Habitat Pieces distance-exit pulse stopped; waiting for a fresh laser sample");
    }
    return;
  }

  if (habitat_update.should_start_habitat_placement) {
    stopAutonomousImuStrafe(context);
    stepper.stop();
    if (!disableMotionActuators(context, front_left, front_right,
                                rear_link, now_ms)) {
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          "Habitat Pieces stopped: placement handoff stop failed",
          robot::EventSource::Uart, now_ms);
      return;
    }
    (void)refreshSharedImuSnapshot(context);
    if (!habitatPlacementStartRequirementsMet(
      ...
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::ConfigurationIncomplete,
          robot::FaultCode::HardwareNotConfigured,
          "Habitat Pieces stopped: Habitat Placement is not ready",
          robot::EventSource::System, now_ms);
      return;
    }
    if (!robot::advanceHabitatCycleAfterPickup(
            context.habitat_cycle)) {
      enterHabitatPiecesFault(
          context, front_left, front_right, rear_link,
          robot::HabitatPiecesStopReason::ConfigurationIncomplete,
          robot::FaultCode::InvalidCommand,
          "Habitat Pieces stopped: invalid pickup-to-placement sequence transition",
          robot::EventSource::System, now_ms);
      return;
    }
    logEvent(context, now_ms, robot::EventSeverity::Info,
             robot::EventSource::System,
             "Habitat Pieces complete; starting Habitat Placement");
  ...
  if (habitat_update.should_drive_forward_to_piece) {
    stopAutonomousImuStrafe(context);
    const robot::FourWheelCommand wheels =
    ...
    if (!applyWheelCommand(context, front_left, front_right, rear_link,
      ...
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          "Habitat Pieces stopped: limit-switch approach command failed",
          robot::EventSource::Uart, now_ms);
      return;
    }
    context.last_command_ms = now_ms;
  ...
  if (habitat_update.should_drive_back_before_lift ||
      habitat_update.should_drive_back_after_pickup) {
    stopAutonomousImuStrafe(context);
    const robot::FourWheelCommand wheels =
        makeHabitatPiecesTranslationCommand(
            context, 0.0F, -1.0F,
    ...
    if (!applyWheelCommand(context, front_left, front_right, rear_link,
      ...
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          habitat_update.should_drive_back_before_lift
              ? "Habitat Pieces stopped: pre-lift reverse command failed"
  ...
  if (habitat_update.should_reacquire_rear_line) {
    const int initial_strafe_direction =
        context.habitat_pieces_config.distance_strafe_direction ==
                robot::HabitatPiecesStrafeDirection::Right
            ? 1
            : -1;
    const AutonomousImuMotionResult result = runAutonomousImuStrafe(
        context, front_left, front_right, rear_link, context.config,
        -initial_strafe_direction,
        context.habitat_pieces_config.rear_line_reacquire_duty, now_ms,
        false);
    if (result != AutonomousImuMotionResult::Running) {
      enterHabitatPiecesImuStrafeFault(
          context, front_left, front_right, rear_link, result, now_ms);
  ...
  const bool pickup_tail_stopped =
      context.habitat_pieces.state ==
  ...
  if (pickup_tail_stopped) {
    ...
    if (!disableMotionActuators(context, front_left, front_right,
      ...
      enterHabitatPiecesFault(
          ...
          robot::HabitatPiecesStopReason::RearCommandFailed,
          robot::FaultCode::CommunicationStale,
          "Habitat Pieces stopped: pickup-tail stop command failed",
          robot::EventSource::Uart, now_ms);
    }
    return;
  }
  ...
  if (!context.last_habitat_pieces_update.should_line_follow) {
    if (context.habitat_pieces.state ==
      ...
      if (reason == robot::HabitatPiecesStopReason::SlideDownTimeout) {
        message = "Habitat Pieces stopped: bottom-limit search timed out";
      } else if (reason == robot::HabitatPiecesStopReason::LiftTimeout) {
        message = "Habitat Pieces stopped: slide lift timed out";
      } else if (reason ==
      ...
      enterHabitatPiecesFault(
          ...
          context.habitat_pieces.timed_out
              ? robot::FaultCode::SearchTimeout
              : robot::FaultCode::InvalidCommand,
          message, robot::EventSource::System, now_ms);
      return;
    }
    stopAutonomousImuStrafe(context);
    disableMotionActuators(context, front_left, front_right, rear_link,
                           now_ms);
    if (context.habitat_pieces.stop_reason ==
        robot::HabitatPiecesStopReason::SlideDownTimeout) {
      setFault(context, robot::FaultCode::SearchTimeout,
               "Habitat Pieces stopped: bottom-limit search timed out");
    } else if (context.habitat_pieces.stop_reason ==
               robot::HabitatPiecesStopReason::LiftTimeout) {
      setFault(context, robot::FaultCode::SearchTimeout,
               "Habitat Pieces stopped: slide lift timed out");
    } else if (context.habitat_pieces.stop_reason ==
               robot::HabitatPiecesStopReason::RearLineTimeout) {
      setFault(context, robot::FaultCode::SearchTimeout,
...
void enterHabitatPlacementImuStrafeFault(
    RuntimeContext& context, DualPwmMotorOutput& front_left,
    DualPwmMotorOutput& front_right, RearCommandLink& rear_link,
    robot::esp2::StepperAxis& stepper,
    const AutonomousImuMotionResult result,
  ...
  enterHabitatPlacementFault(
      ...
      imu_unavailable ||
          ...
          : (link_failed ? robot::FaultCode::CommunicationStale
                         : robot::FaultCode::InvalidCommand),
      imu_unavailable
          ? "Habitat Placement stopped: IMU unavailable during strafe"
          : (link_failed
                 ? "Habitat Placement stopped: IMU strafe command failed"
                 : "Habitat Placement stopped: IMU heading-hold strafe failed"),
      link_failed ? robot::EventSource::Uart
                  : robot::EventSource::System,
      now_ms);
}

void enterHabitatPlacementImuTurnFault(
    RuntimeContext& context, DualPwmMotorOutput& front_left,
    DualPwmMotorOutput& front_right, RearCommandLink& rear_link,
    robot::esp2::StepperAxis& stepper,
    const AutonomousImuMotionResult result,
  ...
  enterHabitatPlacementFault(
      ...
      imu_unavailable || motor_unavailable
          ? robot::FaultCode::HardwareNotConfigured
          : (link_failed ? robot::FaultCode::CommunicationStale
                         : robot::FaultCode::InvalidCommand),
      imu_unavailable
          ? "Habitat Placement stopped: IMU unavailable during turn"
          : (motor_unavailable
                 ? "Habitat Placement stopped: front motor unavailable during turn"
                 : (timed_out
                        ? "Habitat Placement stopped: IMU turn timed out"
                        : (link_failed
                               ? "Habitat Placement stopped: IMU turn command failed"
                               : "Habitat Placement stopped: IMU turn controller failed"))),
      link_failed ? robot::EventSource::Uart
                  : robot::EventSource::System,
      now_ms);
}

AutonomousImuRecoveryUpdate serviceHabitatPlacementImuRecovery(
    RuntimeContext& context,
    const robot::HabitatPlacementState state,
    DualPwmMotorOutput& front_left, DualPwmMotorOutput& front_right,
    RearCommandLink& rear_link, const robot::Milliseconds now_ms) {
  const bool is_turn =
      robot::habitatPlacementStateRequiresImuTurn(state);
  robot::ImuRecoveryState& recovery =
      is_turn ? context.imu_turn_recovery
              : context.imu_strafe_recovery;
  const AutonomousImuRecoveryUpdate recovery_update =
      serviceAutonomousImuRecovery(
          context, recovery, is_turn, front_left, front_right,
          rear_link, context.rear_config, now_ms);
  context.habitat_placement.state_entered_at_ms +=
      recovery_update.timer_adjustment_ms;
  context.habitat_placement.started_at_ms +=
      recovery_update.timer_adjustment_ms;
  return recovery_update;
}
...
void runHabitatPlacement(
  ...
  if (context.habitat_placement.state ==
    ...
    if (!habitatPlacementStartRequirementsMet(
      ...
      enterHabitatPlacementFault(
          ...
          robot::FaultCode::HardwareNotConfigured,
          "Habitat Placement start rejected: configure drive, rear/LSS1 and selected return sensors, IMU turns/strafe, stepper, pusher, duties, angles, and timings",
          robot::EventSource::System, now_ms);
      return;
    }
  ...
  const bool imu_turn_phase = robot::habitatPlacementStateRequiresImuTurn(
      context.habitat_placement.state);
  const bool imu_strafe_phase =
      robot::habitatPlacementStateRequiresImuStrafe(
      context.habitat_placement.state);
  if (imu_turn_phase || imu_strafe_phase) {
    const AutonomousImuRecoveryUpdate recovery_update =
        serviceHabitatPlacementImuRecovery(
            context, context.habitat_placement.state, front_left,
    ...
    if (recovery_update.result ==
      ...
      const AutonomousImuMotionResult result =
          recovery_update.result ==
              ...
              : AutonomousImuMotionResult::CommandFailed;
      if (imu_turn_phase) {
        enterHabitatPlacementImuTurnFault(
            context, front_left, front_right, rear_link, stepper,
            result, now_ms);
      } else {
        enterHabitatPlacementImuStrafeFault(
            context, front_left, front_right, rear_link, stepper,
            result, now_ms);
      }
      return;
  ...
  const bool return_uses_rear_line =
      context.habitat_placement_config.return_line_source ==
      robot::HabitatPlacementReturnLineSource::Rear;
  const bool requires_rear_line_data =
      context.habitat_placement.state ==
          robot::HabitatPlacementState::ReverseLineFollow ||
      (return_uses_rear_line &&
       context.habitat_placement.state ==
           robot::HabitatPlacementState::StrafeRightToReturnLine);
  if (requires_rear_line_data) {
    if (!rear_link.rearLineSnapshotFresh(
      ...
      enterHabitatPlacementFault(
          ...
          return_uses_rear_line &&
                  context.habitat_placement.state ==
                      robot::HabitatPlacementState::
                          StrafeRightToReturnLine
              ? "Habitat Placement stopped: rear-line return data stale"
    ...
    const robot::RearLineSensorSnapshot& rear =
        rear_link.latestRearLineSnapshot();
    const bool final_return_sensors_ready = rear.configured;
    if (!rear.configured ||
        (context.habitat_placement.state ==
                 robot::HabitatPlacementState::ReverseLineFollow
             ? !rear.side_configured
             : !final_return_sensors_ready)) {
      enterHabitatPlacementFault(
          context, front_left, front_right, rear_link, stepper,
          robot::HabitatPlacementFaultReason::HardwareNotReady,
          robot::FaultCode::HardwareNotConfigured,
          context.habitat_placement.state ==
                  robot::HabitatPlacementState::StrafeRightToReturnLine
              ? "Habitat Placement stopped: rear return-line sensors unavailable"
  ...
  if ((context.habitat_placement.state ==
           robot::HabitatPlacementState::ForwardToSlide ||
       context.habitat_placement.state ==
  ...
  if (update.faulted) {
    ...
    const bool return_timeout =
        update.fault_reason ==
        robot::HabitatPlacementFaultReason::ReturnLineTimeout;
    ...
    enterHabitatPlacementFault(
        ...
        lss_timeout
            ...
            : (return_timeout
                   ? (return_uses_rear_line
                          ? "Habitat Placement stopped: rear-line return strafe timed out"
                          : "Habitat Placement stopped: front-line return strafe timed out")
                   : (stepper_timeout
  ...
  if (update.state != previous_state) {
    ...
    const bool entered_imu_turn =
        robot::habitatPlacementStateRequiresImuTurn(update.state);
    const bool entered_imu_strafe =
        robot::habitatPlacementStateRequiresImuStrafe(update.state);
    if (entered_imu_turn || entered_imu_strafe) {
      // This first-iteration gate closes the handoff window between the state
      // update above and the next loop's normal per-phase recovery service.
      const AutonomousImuRecoveryUpdate recovery_update =
          serviceHabitatPlacementImuRecovery(
              context, update.state, front_left, front_right, rear_link,
              now_ms);
      if (recovery_update.result ==
          AutonomousImuRecoveryResult::Paused) {
        return;
  ...
  if (update.should_turn_to_initial_heading ||
    ...
    if (result != AutonomousImuMotionResult::Running &&
      ...
      enterHabitatPlacementImuTurnFault(
          context, front_left, front_right, rear_link, stepper, result,
          now_ms);
    }
    return;
  }

  const bool should_imu_strafe =
  ...
  if (should_imu_strafe) {
    stopAutonomousImuTurn(context);
    const int lateral_direction =
        update.should_strafe_left_after_clockwise ? -1 : 1;
    const float strafe_duty =
        update.should_strafe_right_before_counter_clockwise
            ? context.habitat_placement_config
            ...
            : ((update.should_strafe_left_after_clockwise ||
                update.should_strafe_right_after_clockwise)
                   ...
                   : context.habitat_placement_config.strafe_right_duty);
    const AutonomousImuMotionResult result = runAutonomousImuStrafe(
        context, front_left, front_right, rear_link, context.rear_config,
        lateral_direction, strafe_duty, now_ms, false);
    if (result != AutonomousImuMotionResult::Running) {
      enterHabitatPlacementImuStrafeFault(
          context, front_left, front_right, rear_link, stepper, result,
          now_ms);
  ...
  if (update.complete) {
    ...
    if (context.habitat_cycle.phase ==
        robot::HabitatCyclePhase::Placement) {
      if (!robot::advanceHabitatCycleAfterPlacement(
              context.habitat_cycle)) {
        enterHabitatPlacementFault(
            context, front_left, front_right, rear_link, stepper,
            robot::HabitatPlacementFaultReason::HardwareNotReady,
            robot::FaultCode::InvalidCommand,
            "Habitat Placement stopped: invalid placement-to-pickup sequence transition",
            robot::EventSource::System, now_ms);
        return;
      }
      if (context.habitat_cycle.phase ==
        ...
        logEvent(context, now_ms, robot::EventSeverity::Info,
                 robot::EventSource::System,
                 "Habitat Placement complete; starting next Habitat Pieces pickup");
        ...
        logEvent(context, now_ms, robot::EventSeverity::Info,
                 robot::EventSource::System,
                 "Habitat pickup/placement sequence complete");
      }
    }
  }
}
...
void runPegFinder(RuntimeContext& context,
  ...
  switch (context.peg_finder.state) {
    case robot::PegFinderState::WaitForStart: {
      ...
      if (!pegFinderStartRequirementsMet(front_left, front_right, rear_link,
        ...
        enterPegFinderFault(
            ...
            robot::FaultCode::HardwareNotConfigured,
            "PegFinder start rejected: configure drive, funnel, IMU turn tuning, clockwise angle, GPIO 47 limit, all claw open/closed angles, timings, and ESP1 link",
            robot::EventSource::System);
        return;
      }
      context.last_imu_turn_update = {};
    ...
    case robot::PegFinderState::ShakeLeftAfterClaw3:
    case robot::PegFinderState::ShakeRightAfterClaw3:
    case robot::PegFinderState::PostShakeAfterClaw3Delay:
  ...
  if (update.state != previous_state) {
    ...
    switch (update.state) {
      ...
      case robot::PegFinderState::OpenClaw3: {
        ...
        servo_result = claws.commandExclusiveOpenClaw(claw_index);
        break;
      }
      case robot::PegFinderState::WaitForStart:
      case robot::PegFinderState::RotateClockwise:
    ...
    switch (update.state) {
      ...
      case robot::PegFinderState::OpenClaw3:
        message = update.should_open_claw_1
                      ? "PegFinder claw 1 opened; claws 2 and 3 closed"
                      : (update.should_open_claw_2
                             ? "PegFinder claw 2 opened; claws 1 and 3 closed"
                             : "PegFinder claw 3 opened; claws 1 and 2 closed");
        break;
      case robot::PegFinderState::PostClaw1OpenDelay:
      case robot::PegFinderState::PostClaw2OpenDelay:
        break;
      ...
      case robot::PegFinderState::PostShakeAfterClaw3Delay:
        ...
        break;
      case robot::PegFinderState::PostClawsOpenDelay:
        message = context.peg_finder_config.shake_duty > 0.0F
                      ? "PegFinder third exclusive claw selection complete; pre-shake delay started"
                      : "PegFinder third exclusive claw selection complete; funnel reverse delay started";
        break;
      case robot::PegFinderState::FunnelReverse:
...
void runSolarPanelAutonomy(RuntimeContext& context,
                           ...
                           RearCommandLink& rear_link,
                           const robot::Milliseconds now_ms) {
  // The funnel has its own timeline. Service it before the chassis
  // IMU-recovery gate so a paused strafe cannot let its command heartbeat
  // expire while its configured opening duration keeps advancing.
  if (context.solar_completion_mechanisms_started &&
      !context.solar_funnel_open_complete &&
      context.autonomous_state !=
          robot::SolarPanelAutonomyState::SolarSearchFault) {
    const SolarCompletionMechanismResult mechanism_result =
        serviceSolarCompletionMechanisms(context, rear_link, now_ms);
    if (mechanism_result ==
        SolarCompletionMechanismResult::HardwareUnavailable) {
      enterSolarPanelSearchFault(
          context, front_left, front_right, rear_link, now_ms,
          robot::SolarPanelFaultReason::RearLinkStale,
          robot::FaultCode::CommunicationStale,
          "solar completion mechanisms stopped: ESP1 status stale or mechanism hardware unavailable",
          robot::EventSource::Uart);
      return;
    }
    if (mechanism_result ==
        SolarCompletionMechanismResult::FunnelCommandFailed) {
      enterSolarPanelSearchFault(
          context, front_left, front_right, rear_link, now_ms,
          robot::SolarPanelFaultReason::FunnelCommandFailed,
          robot::FaultCode::CommunicationStale,
          "solar completion mechanisms stopped: funnel command failed",
          robot::EventSource::Uart);
      return;
    }
    if (mechanism_result == SolarCompletionMechanismResult::Complete) {
      logEvent(context, now_ms, robot::EventSeverity::Info,
               robot::EventSource::Motor,
               context.solar_hook_commanded_open
                   ? "solar funnel opening complete; hook remains raised/open"
                   : "solar funnel opening complete; hook remains closed until front tape detection");
    }
  }
  ...
  switch (context.autonomous_state) {
    ...
    case robot::SolarPanelAutonomyState::
      ...
      const bool contact_timeout_fallback =
          sequence_update.transitioned &&
          sequence_update.next_state ==
      ...
      if (sequence_update.next_state ==
        ...
        enterSolarPanelSearchFault(
            context, front_left, front_right, rear_link, now_ms,
            robot::SolarPanelFaultReason::LineReacquireTimeout,
            robot::FaultCode::SearchTimeout,
            "solar left strafe timed out before the front line was found",
            robot::EventSource::System);
        return;
      }
      if (sequence_update.transitioned) {
        stopAutonomousImuStrafe(context);
        const bool motion_stopped = disableMotionActuators(
            context, front_left, front_right, rear_link, now_ms);
        const bool funnel_opening_starting =
            !context.solar_completion_mechanisms_started &&
            (sequence_update.next_state ==
                 robot::SolarPanelAutonomyState::StrafeLeftToFrontLine ||
             sequence_update.next_state ==
                 robot::SolarPanelAutonomyState::
                     ForwardLineFollowAfterFrontDetection);
        if (funnel_opening_starting) {
          const SolarCompletionMechanismResult mechanism_result =
              startSolarFunnelOpening(context, rear_link, now_ms);
          if (mechanism_result ==
              SolarCompletionMechanismResult::HardwareUnavailable) {
            enterSolarPanelSearchFault(
                context, front_left, front_right, rear_link, now_ms,
                robot::SolarPanelFaultReason::HardwareNotReady,
                robot::FaultCode::HardwareNotConfigured,
                "solar line return stopped: solar hook or funnel unavailable",
                robot::EventSource::System);
            return;
          }
          if (mechanism_result ==
            ...
            enterSolarPanelSearchFault(
                ...
                robot::SolarPanelFaultReason::FunnelCommandFailed,
                robot::FaultCode::CommunicationStale,
                "solar line return stopped: funnel-open command failed",
                robot::EventSource::Uart);
            return;
          }
          logEvent(
              context, now_ms, robot::EventSeverity::Info,
              robot::EventSource::Motor,
              "timed solar funnel opening started with the line return; hook remains closed until front tape detection");
        }
        if (robot::shouldOpenSolarHookAfterLineDetection(sequence_update)) {
          if (!motion_stopped) {
            enterSolarPanelSearchFault(
                context, front_left, front_right, rear_link, now_ms,
                robot::SolarPanelFaultReason::RearLinkStale,
                robot::FaultCode::CommunicationStale,
                "solar front-line handoff stopped: failed to stop chassis before opening hook",
                robot::EventSource::Uart);
            return;
          }
          const SolarCompletionMechanismResult hook_result =
              openSolarHookForMission(context, rear_link, now_ms);
          if (hook_result ==
              SolarCompletionMechanismResult::HardwareUnavailable) {
            enterSolarPanelSearchFault(
                context, front_left, front_right, rear_link, now_ms,
                robot::SolarPanelFaultReason::HardwareNotReady,
                robot::FaultCode::HardwareNotConfigured,
                "solar front-line handoff stopped: solar hook unavailable",
                robot::EventSource::System);
            return;
          }
          if (hook_result ==
              SolarCompletionMechanismResult::SolarHookCommandFailed) {
            enterSolarPanelSearchFault(
                context, front_left, front_right, rear_link, now_ms,
                robot::SolarPanelFaultReason::SolarHookCommandFailed,
                robot::FaultCode::CommunicationStale,
                "solar front-line handoff stopped: failed to raise/open solar hook",
                robot::EventSource::Uart);
            return;
          }
        }
        ...
        if (sequence_update.next_state ==
          ...
          logEvent(context, now_ms, robot::EventSeverity::Info,
                   robot::EventSource::Line,
                   "solar front tape detected; strafe stopped, hook raised/open, and forward line-follow PID started");
          printTelemetry(context, sensors, rear_link, now_ms);
          return;
        }
        const char* message = "solar contact adjustment state changed";
        if (contact_timeout_fallback) {
          message =
              "solar contact timeout; continuing post-contact forward motion";
        ...
        logEvent(context, now_ms,
                 contact_timeout_fallback
                     ? robot::EventSeverity::Warn
                     : robot::EventSeverity::Info,
    ...
    case robot::SolarPanelAutonomyState::FrontLineFollowComplete: {
      if (!disableMotionActuators(
        ...
        enterSolarPanelSearchFault(
            ...
            robot::SolarPanelFaultReason::RearLinkStale,
            robot::FaultCode::CommunicationStale,
            "solar completion stopped: failed to hold chassis stopped",
            robot::EventSource::Uart);
        return;
      }
      if (!context.solar_completion_mechanisms_started) {
        enterSolarPanelSearchFault(
            ...
            robot::SolarPanelFaultReason::HardwareNotReady,
            robot::FaultCode::InvalidCommand,
            "solar completion stopped: funnel opening was not started",
            robot::EventSource::System);
        return;
      }
      if (!context.solar_hook_commanded_open ||
          !context.solar_hook_hold_open_for_mission) {
        enterSolarPanelSearchFault(
            context, front_left, front_right, rear_link, now_ms,
            robot::SolarPanelFaultReason::SolarHookCommandFailed,
            robot::FaultCode::InvalidCommand,
            "solar completion stopped: hook was not opened after front-line detection",
            robot::EventSource::System);
        return;
      }
      if (context.solar_funnel_open_complete) {
        ...
        logEvent(context, now_ms, robot::EventSeverity::Info,
                 robot::EventSource::System,
                 "solar run complete: line return and funnel opening finished; hook remains raised/open");
        return;
      }
      enterSolarPanelAutonomyState(
          context,
      ...
      logEvent(context, now_ms, robot::EventSeverity::Info,
               robot::EventSource::System,
               "solar route complete; hook held open while waiting for funnel opening");
      return;
    }
...
void fillTelemetrySnapshot(const RuntimeContext& context,
                           ...
                           const RearCommandLink& rear_link,
                           const ClawServoBank& claws,
                           const DigitalActiveHighLimitSwitch&
                               habitat_piece_limit,
                           const DigitalActiveHighLimitSwitch& funnel_limit,
                           const robot::esp2::ImuAcquisitionSnapshot& imu,
  ...
  snapshot.solar_funnel_open_elapsed_ms =
      context.solar_completion_mechanisms_started
          ? elapsedSince(now_ms,
                         context.solar_funnel_open_started_at_ms)
          : 0U;
  if (snapshot.solar_funnel_open_elapsed_ms >
      snapshot.solar_funnel_open_duration_ms) {
    snapshot.solar_funnel_open_elapsed_ms =
        snapshot.solar_funnel_open_duration_ms;
  }
  ...
  snapshot.habitat_pieces_post_count_stop_delay_ms =
      context.habitat_pieces_config.post_count_stop_delay_ms;
  snapshot.habitat_pieces_post_count_stop_elapsed_ms =
      context.habitat_pieces.post_count_stop_elapsed_ms;
  ...
  snapshot.habitat_pieces_exit_strafe_pulse_ms =
      context.habitat_pieces_config.exit_strafe_pulse_ms;
  snapshot.habitat_pieces_exit_strafe_pulse_elapsed_ms =
      context.habitat_pieces.exit_strafe_pulse_elapsed_ms;
  ...
  snapshot.habitat_pieces_approach_timeout_ms =
      context.habitat_pieces_config.approach_timeout_ms;
  snapshot.habitat_pieces_approach_elapsed_ms =
      context.habitat_pieces.approach_elapsed_ms;
  snapshot.habitat_pieces_pre_lift_reverse_duration_ms =
      context.habitat_pieces_config.pre_lift_reverse_duration_ms;
  snapshot.habitat_pieces_pre_lift_reverse_elapsed_ms =
      context.habitat_pieces.pre_lift_reverse_elapsed_ms;
  snapshot.habitat_pieces_lift_steps =
      context.habitat_pieces_config.lift_steps;
  snapshot.habitat_pieces_lift_speed_steps_per_second =
      context.habitat_pieces_config.lift_speed_steps_per_second;
  snapshot.habitat_pieces_lift_timeout_ms =
      context.habitat_pieces_config.lift_timeout_ms;
  snapshot.habitat_pieces_lift_elapsed_ms =
      context.habitat_pieces.lift_elapsed_ms;
  snapshot.habitat_pieces_lift_start_delay_ms =
      context.habitat_pieces_config.lift_start_delay_ms;
  snapshot.habitat_pieces_lift_start_delay_elapsed_ms =
      context.habitat_pieces.lift_start_delay_elapsed_ms;
  snapshot.habitat_pieces_post_pickup_reverse_duty =
      context.habitat_pieces_config.post_pickup_reverse_duty;
  snapshot.habitat_pieces_post_pickup_reverse_duration_ms =
      context.habitat_pieces_config
          .post_pickup_reverse_duration_ms;
  ...
  snapshot.habitat_pieces_rear_line_reacquire_duty =
      context.habitat_pieces_config.rear_line_reacquire_duty;
  snapshot.habitat_pieces_rear_line_reacquire_timeout_ms =
      context.habitat_pieces_config.rear_line_reacquire_timeout_ms;
  snapshot.habitat_pieces_rear_line_reacquire_elapsed_ms =
      context.habitat_pieces.rear_line_reacquire_elapsed_ms;
  ...
  snapshot.habitat_pieces_distance_exit_pulse_count =
      context.habitat_pieces.distance_exit_pulse_count;
  snapshot.habitat_pieces_configuration_valid =
      g_runtime.stepper != nullptr &&
      habitatPiecesProfileConfigValid(
          context, context.habitat_pieces_config,
          *g_runtime.stepper);
  snapshot.habitat_pieces_all_profiles_valid =
      g_runtime.stepper != nullptr &&
      allHabitatPiecesProfilesValid(context, *g_runtime.stepper);
  snapshot.habitat_pieces_start_ready =
      g_runtime.stepper != nullptr && habitatSequenceStartRequirementsMet(
          sensors, front_left, front_right, rear_link,
          habitat_piece_limit, claws, *g_runtime.stepper, now_ms,
          context);
  snapshot.habitat_pieces_editor_profile_number =
      static_cast<std::uint8_t>(
          context.habitat_pieces_editor_profile_index + 1U);
  const bool habitat_sequence_has_profile =
      context.habitat_cycle.phase != robot::HabitatCyclePhase::Idle;
  const std::uint8_t displayed_habitat_profile_index =
      habitat_sequence_has_profile
          ? context.habitat_cycle.active_profile_index
          : context.habitat_pieces_editor_profile_index;
  snapshot.habitat_pieces_active_profile_number =
      static_cast<std::uint8_t>(displayed_habitat_profile_index + 1U);
  snapshot.habitat_pieces_completed_profile_count =
      context.habitat_cycle.completed_profile_count;
  snapshot.habitat_pieces_sequence_active =
      robot::habitatCycleActive(context.habitat_cycle);
  snapshot.habitat_cycle_phase = context.habitat_cycle.phase;
  snapshot.habitat_pieces_approach_limit_configured =
      habitat_piece_limit.configured();
  snapshot.habitat_pieces_approach_limit_raw_level =
      habitat_piece_limit.rawLevel();
  snapshot.habitat_pieces_approach_limit_active =
      habitat_piece_limit.active();
  ...
  snapshot.habitat_pieces_approach_limit_reached =
      context.habitat_pieces.approach_limit_reached;
  snapshot.habitat_pieces_lift_complete =
      context.habitat_pieces.lift_complete;
  snapshot.habitat_pieces_rear_line_detected =
      context.habitat_pieces.rear_line_latched;
  ...
  snapshot.habitat_pieces_distance_exit_above_threshold =
      context.habitat_pieces.distance_exit_above_threshold;
  ...
  snapshot.habitat_placement_configuration_valid =
      ...
      habitatPlacementProfileConfigValid(
          context, context.habitat_placement_config,
          *g_runtime.stepper);
  snapshot.habitat_placement_all_profiles_valid =
      g_runtime.stepper != nullptr &&
      allHabitatPlacementProfilesValid(context, *g_runtime.stepper);
  ...
  snapshot.habitat_placement_editor_profile_number =
      static_cast<std::uint8_t>(
          context.habitat_placement_editor_profile_index + 1U);
  const std::uint8_t displayed_placement_profile_index =
      habitat_sequence_has_profile
          ? context.habitat_cycle.active_profile_index
          : context.habitat_placement_editor_profile_index;
  snapshot.habitat_placement_active_profile_number =
      static_cast<std::uint8_t>(
          displayed_placement_profile_index + 1U);
  snapshot.habitat_placement_completed_profile_count =
      context.habitat_cycle.completed_profile_count;
  snapshot.habitat_placement_sequence_active =
      robot::habitatCycleActive(context.habitat_cycle);
  ...
  snapshot.peg_finder_shaking_left =
      ...
      (context.peg_finder.state ==
       ...
       context.peg_finder.state ==
           robot::PegFinderState::ShakeLeftAfterClaw3);
  snapshot.peg_finder_shaking_right =
      ...
      (context.peg_finder.state ==
       ...
       context.peg_finder.state ==
           robot::PegFinderState::ShakeRightAfterClaw3);
...
bool runtimeReady() {
  return g_runtime.context != nullptr && g_runtime.sensors != nullptr &&
         g_runtime.habitat_piece_limit != nullptr &&
...
<main>
  ...
  <section>
    ...
    <div class="kv">
      ...
      <span>Post-count exit</span><span id="habitatDistanceExit" class="mono"></span>
      <span>Slide down</span><span id="habitatSlideDown" class="mono"></span>
      <span>Limit-switch approach</span><span id="habitatApproach" class="mono"></span>
      <span>Pre-lift reverse</span><span id="habitatPreLiftReverse" class="mono"></span>
      <span>Concurrent lift</span><span id="habitatLift" class="mono"></span>
      <span>Lift-start stop delay</span><span id="habitatLiftDelay" class="mono"></span>
      <span>Post-pickup reverse</span><span id="habitatPickupReverse" class="mono"></span>
      <span>Rear-line reacquisition</span><span id="habitatRearLine" class="mono"></span>
    ...
    <div class="two">
      <label>Edit pickup profile
        <select id="habitatProfile" onchange="habitatSelectProfile()">
      ...
      <label>Distance strafe timeout, then run exit sequence (ms) <input id="habitatDistanceTimeout" type="number" min="1" max="30000" step="100"></label>
      ...
      <label>Exit strafe pulse (ms) <input id="habitatExitPulse" type="number" min="1" max="30000" step="10"></label>
      <label>Slide-down speed (steps/s) <input id="habitatSlideDownSpeed" type="number" min="1" step="1"></label>
      <label>Slide-down timeout (ms) <input id="habitatSlideDownTimeout" type="number" min="1" max="30000" step="100"></label>
      <label>Approach forward duty <input id="habitatApproachDuty" type="number" min="0.001" max="1" step="0.01"></label>
      <label>Approach timeout, then continue pickup (ms) <input id="habitatApproachTimeout" type="number" min="1" max="30000" step="100"></label>
      <label>Pre-lift reverse duration (ms) <input id="habitatPreLiftReverseDuration" type="number" min="1" max="30000" step="10"></label>
      <label>Lift distance (steps) <input id="habitatLiftSteps" type="number" min="1" step="1"></label>
      <label>Lift speed (steps/s) <input id="habitatLiftSpeed" type="number" min="1" step="1"></label>
      <label>Lift timeout (ms) <input id="habitatLiftTimeout" type="number" min="1" max="30000" step="100"></label>
      <label>Stopped delay after lift starts (ms) <input id="habitatLiftDelayInput" type="number" min="1" max="30000" step="100"></label>
      <label>Post-pickup reverse duty <input id="habitatPickupReverseDuty" type="number" min="0.001" max="1" step="0.01"></label>
      <label>Post-pickup reverse duration (ms) <input id="habitatPickupReverseDuration" type="number" min="1" max="30000" step="100"></label>
      <label>Rear-line strafe duty <input id="habitatRearLineDuty" type="number" min="0.001" max="1" step="0.01"></label>
      <label>Rear-line strafe timeout (ms) <input id="habitatRearLineTimeout" type="number" min="1" max="30000" step="100"></label>
    </div>
    <div class="row">
  ...
  <section>
    ...
    <div class="muted">Each placement profile runs immediately after its matching pickup profile. The return-line source is selectable per profile; the intended defaults are Front, Front, then Rear. Every placement captures its IMU heading at its own start, and the slide begins lowering concurrently with the forward-to-slide drive.</div>
    <div class="kv">
      ...
      <span>Profile sequence</span><span id="placementProfiles" class="mono"></span>
      <span>Return line sensor</span><span id="placementReturnSensor" class="mono"></span>
    ...
    <div class="two">
      <label>Edit matching placement profile
        <select id="placementProfile" onchange="placementSelectProfile()">
          <option value="1">Profile 1</option>
          <option value="2">Profile 2</option>
          <option value="3">Profile 3</option>
        </select>
      </label>
      <label>Return line source
        <select id="placementReturnSource">
          <option value="FRONT">Front sensors</option>
          <option value="REAR">Rear sensors</option>
        </select>
      </label>
      ...
      <label>After-CW strafe-right ms (same duty) <input id="placementAfterCwStrafeRightMs" type="number" min="1" max="30000" step="100"></label>
  ...
  <section>
    ...
    <div class="muted">Stage the robot with the slider up, funnel mechanically closed, and solar hook down/closed. Solar explicitly holds the hook closed at its start. Front-only panel contact starts one retry; no-front contact or an expired retry continues into the post-contact forward drive. During the final left strafe Solar opens the funnel while keeping the hook closed. Front-line detection stops the strafe, then raises and holds the hook before advancing immediately; that timeout only bounds a failed line search.</div>
    <div class="kv">
    ...
    <div class="two">
      ...
      <label>Contact timeout, then continue forward (ms) <input id="solarContactTimeoutMs" type="number" min="1" step="500"></label>
      ...
      <label>Retry right timeout, then continue forward (ms) <input id="solarRetryStrafeTimeoutMs" type="number" min="1" step="500"></label>
      ...
      <label>Front-line reacquire timeout ms <input id="solarLineReacquireTimeoutMs" type="number" min="1" max="30000" step="100"></label>
  ...
  <section>
    <h2>PegFinder</h2>
    <div class="muted">IMU-controlled clockwise turn, then timed backward and forward chassis movements, followed by funnel forward until the active-high GPIO 47 limit is pressed. The turn uses the shared IMU Turn tuning and the angle below. After a delay, PegFinder selects claws in the configured order using the shared Servo Control panel angles: the selected claw opens while the other two close. After the third selection, PegFinder waits, then reverses the funnel for the configured duration.</div>
    <div class="kv">
    ...
    <div class="two">
      ...
      <label>Delay after shake before next claw/funnel (ms) <input id="pegFinderPostShakeDelayMs" type="number" min="0" max="30000" step="10"></label>
      ...
      <label>Delay after third claw selection ms <input id="pegFinderPostClawsOpenDelayMs" type="number" min="1" step="100"></label>
...
function loadHabitatControls(j){
  ...
  setLfValue('habitatPostCountDelay',
             h.post_count_stop_delay_ms || '');
  setLfValue('habitatExitDuty', h.exit_strafe_duty || '');
  setLfValue('habitatExitPulse', h.exit_strafe_pulse_ms || '');
  setLfValue('habitatSlideDownSpeed',
             h.slide_down_speed_steps_per_second || '');
  setLfValue('habitatSlideDownTimeout', h.slide_down_timeout_ms || '');
  setLfValue('habitatApproachDuty', h.approach_forward_duty || '');
  setLfValue('habitatApproachTimeout', h.approach_timeout_ms || '');
  setLfValue('habitatPreLiftReverseDuration',
             h.pre_lift_reverse_duration_ms || '');
  setLfValue('habitatLiftSteps', h.lift_steps || '');
  setLfValue('habitatLiftSpeed',
             h.lift_speed_steps_per_second || '');
  setLfValue('habitatLiftTimeout', h.lift_timeout_ms || '');
  setLfValue('habitatLiftDelayInput', h.lift_start_delay_ms || '');
  setLfValue('habitatPickupReverseDuty',
             h.post_pickup_reverse_duty || '');
  setLfValue('habitatPickupReverseDuration',
             h.post_pickup_reverse_duration_ms || '');
  setLfValue('habitatRearLineDuty', h.rear_line_reacquire_duty || '');
  setLfValue('habitatRearLineTimeout',
             h.rear_line_reacquire_timeout_ms || '');
  habitatLoaded = true;
}
function habitatParams(){
  return new URLSearchParams({
    ...
    'exit-strafe-pulse-ms': qs('habitatExitPulse').value,
    'slide-down-speed': qs('habitatSlideDownSpeed').value,
    'slide-down-timeout-ms': qs('habitatSlideDownTimeout').value,
    'approach-duty': qs('habitatApproachDuty').value,
    'approach-timeout-ms': qs('habitatApproachTimeout').value,
    'pre-lift-reverse-duration-ms': qs('habitatPreLiftReverseDuration').value,
    'lift-steps': qs('habitatLiftSteps').value,
    'lift-speed': qs('habitatLiftSpeed').value,
    'lift-timeout-ms': qs('habitatLiftTimeout').value,
    'lift-start-delay-ms': qs('habitatLiftDelayInput').value,
    'post-pickup-reverse-duty': qs('habitatPickupReverseDuty').value,
    'post-pickup-reverse-duration-ms': qs('habitatPickupReverseDuration').value,
    'rear-line-duty': qs('habitatRearLineDuty').value,
    'rear-line-timeout-ms': qs('habitatRearLineTimeout').value
  });
}
function habitatSelectProfile(){
  habitatLoaded = false;
  return api(`/api/autonomous/habitat-pieces/profile?profile=${encodeURIComponent(qs('habitatProfile').value)}`)
...
function loadPlacementControls(j){
  ...
  setLfValue('placementProfile', p.editor_profile || 1);
  setLfValue('placementReturnSource', p.return_line_source || 'FRONT');
  const values = {
    ...
    placementAfterCwStrafeRightMs:p.post_clockwise_strafe_right_duration_ms,
...
function placementParams(){
  return new URLSearchParams({
    profile:qs('placementProfile').value,
    'return-line-source':qs('placementReturnSource').value,
    ...
    'post-cw-strafe-right-ms':qs('placementAfterCwStrafeRightMs').value,
...
function placementSelectProfile(){
  placementLoaded = false;
  return api(`/api/autonomous/habitat-placement/profile?profile=${encodeURIComponent(qs('placementProfile').value)}`)
...
function loadSolarControls(j){
  ...
  setLfValue('solarLineReacquireTimeoutMs', a.line_reacquire_strafe_timeout_ms);
...
function solarParams(){
  ...
  add('line-reacquire-timeout-ms', 'solarLineReacquireTimeoutMs');
...
function update(){
  fetch('/api/telemetry').then(r => r.json()).then(j => {
    ...
    qs('habitatProfiles').textContent =
      `active ${habitat.active_profile ?? 1} / ${habitat.profile_count ?? 3} · completed ${habitat.completed_profile_count ?? 0} · phase=${habitat.sequence_phase || 'IDLE'} · all configured=${yn(habitat.all_profiles_valid)} · running=${yn(habitat.sequence_active)}`;
    qs('habitatProfiles').className =
      habitat.all_profiles_valid ? 'mono good' : 'mono bad';
    ...
    qs('habitatDistance').textContent = habitat.distance_measurement_available
      ? `${habitat.distance_substituted_no_target ? 'N/A → ' : ''}${habitat.distance_mm ?? 0} / ${habitat.distance_threshold_mm ?? 0} mm · zone=${habitat.distance_zone_active ? 'AT/BELOW' : 'ABOVE'} · new=${yn(habitat.distance_sample_new)}`
      : `NO FRESH SAMPLE / ${habitat.distance_threshold_mm ?? 0} mm · zone latched=${yn(habitat.distance_zone_active)}`;
    ...
    qs('habitatDistanceExit').textContent =
      `delay ${habitat.post_count_stop_elapsed_ms ?? 0} / ${habitat.post_count_stop_delay_ms ?? 0} ms (${habitat.post_count_stop_remaining_ms ?? 0} ms remaining) · pulse duty ${Number(habitat.exit_strafe_duty ?? 0).toFixed(3)} for ${habitat.exit_strafe_pulse_elapsed_ms ?? 0} / ${habitat.exit_strafe_pulse_ms ?? 0} ms (${habitat.exit_strafe_pulse_remaining_ms ?? 0} ms remaining) · pulses=${habitat.distance_exit_pulse_count ?? 0} · waiting=${yn(habitat.post_count_waiting)} · strafing=${yn(habitat.exit_strafe_pulsing)} · checking=${yn(habitat.exit_distance_checking)} · above=${yn(habitat.distance_exit_above_threshold)}`;
    ...
    qs('habitatApproach').textContent =
      `GPIO48 ${habitat.approach_limit_raw_level === 1 ? 'HIGH' : (habitat.approach_limit_raw_level === 0 ? 'LOW' : 'UNKNOWN')} · configured=${yn(habitat.approach_limit_configured)} · pressed=${yn(habitat.approach_limit_active)} · latched=${yn(habitat.approach_limit_reached)} · ${habitat.approach_elapsed_ms ?? 0} / ${habitat.approach_timeout_ms ?? 0} ms at duty ${Number(habitat.approach_forward_duty ?? 0).toFixed(3)} · driving=${yn(habitat.approaching_piece)}`;
    qs('habitatApproach').className =
      !habitat.approach_limit_configured ? 'mono bad' : (habitat.approach_limit_active || habitat.approaching_piece ? 'mono good' : 'mono muted');
    qs('habitatPreLiftReverse').textContent =
      `${habitat.pre_lift_reverse_elapsed_ms ?? 0} / ${habitat.pre_lift_reverse_duration_ms ?? 0} ms at duty ${Number(habitat.post_pickup_reverse_duty ?? 0).toFixed(3)} · active=${yn(habitat.pre_lift_reversing)}`;
    qs('habitatPreLiftReverse').className =
      habitat.pre_lift_reversing ? 'mono good' : 'mono muted';
    qs('habitatLift').textContent =
      `${habitat.lift_steps ?? 0} steps at ${habitat.lift_speed_steps_per_second ?? 0} steps/s · ${habitat.lift_elapsed_ms ?? 0} / ${habitat.lift_timeout_ms ?? 0} ms · active=${yn(habitat.lifting_slide)} · complete=${yn(habitat.lift_complete)}`;
    qs('habitatLift').className =
      habitat.lift_complete ? 'mono good' : (habitat.lifting_slide ? 'mono good' : 'mono muted');
    qs('habitatLiftDelay').textContent =
      `${habitat.lift_start_delay_elapsed_ms ?? 0} / ${habitat.lift_start_delay_ms ?? 0} ms · stopped=${yn(habitat.lift_start_waiting)}`;
    qs('habitatLiftDelay').className =
      habitat.lift_start_waiting ? 'mono good' : 'mono muted';
    qs('habitatPickupReverse').textContent =
      `${habitat.post_pickup_reverse_elapsed_ms ?? 0} / ${habitat.post_pickup_reverse_duration_ms ?? 0} ms at duty ${Number(habitat.post_pickup_reverse_duty ?? 0).toFixed(3)} · active=${yn(habitat.post_pickup_reversing)}`;
    qs('habitatPickupReverse').className =
      habitat.post_pickup_reversing ? 'mono good' : 'mono muted';
    qs('habitatRearLine').textContent =
      `${habitat.rear_line_reacquire_elapsed_ms ?? 0} / ${habitat.rear_line_reacquire_timeout_ms ?? 0} ms · opposite ${habitat.distance_strafe_direction || 'NONE'} at duty ${Number(habitat.rear_line_reacquire_duty ?? 0).toFixed(3)} · detected=${yn(habitat.rear_line_detected)} · active=${yn(habitat.reacquiring_rear_line)} · waiting for lift=${yn(habitat.waiting_for_lift)}`;
    qs('habitatRearLine').className =
      habitat.rear_line_detected ? 'mono good' : (habitat.reacquiring_rear_line ? 'mono good' : 'mono muted');
    ...
    qs('placementProfiles').textContent =
      `active ${placement.active_profile ?? 1} / ${placement.profile_count ?? 3} · completed ${placement.completed_profile_count ?? 0} · all configured=${yn(placement.all_profiles_valid)} · running=${yn(placement.sequence_active)}`;
    qs('placementProfiles').className =
      placement.all_profiles_valid ? 'mono good' : 'mono bad';
    qs('placementReturnSensor').textContent =
      placement.return_line_source || 'FRONT';
    ...
    qs('autoMechanisms').textContent =
      `hook=${solarHook.commandedOpen ? 'OPEN/UP' : 'CLOSED/DOWN'}${hookOutput} · funnel ${funnelDirection} duty=${funnelOpenDuty.toFixed(3)} · ${a.funnel_open_elapsed_ms ?? 0} / ${a.funnel_open_duration_ms ?? 0} ms · funnel active=${yn(a.opening_hook_and_funnel)}`;
...
void handleMode() {
  ...
  cancelHabitatCycle(*g_runtime.context);
...
void handleSensors() {
  ...
  std::snprintf(g_json_buffer, sizeof(g_json_buffer),
                ...
                "\"limit_switch_habitat_piece_configured\":%s,"
                "\"limit_switch_habitat_piece_raw_level\":%d,"
                "\"limit_switch_habitat_piece_active\":%s,"
...
void handleHabitatPiecesProfileSelect() {
  ...
  RuntimeContext& context = *g_runtime.context;
  if (robot::habitatCycleActive(context.habitat_cycle) ||
      (context.habitat_pieces.state !=
           robot::HabitatPiecesState::WaitForStart &&
       context.habitat_pieces.state !=
           robot::HabitatPiecesState::Complete &&
       context.habitat_pieces.state !=
           robot::HabitatPiecesState::Fault)) {
    sendErrorJson(409, "stop the Habitat sequence before selecting a profile");
    return;
  }
  robot::Milliseconds profile_number =
      static_cast<robot::Milliseconds>(
          context.habitat_pieces_editor_profile_index + 1U);
  if (!g_server.hasArg("profile") ||
      !argUnsigned("profile", profile_number, profile_number, true) ||
      profile_number == 0U ||
      profile_number > robot::kHabitatProfileCount) {
    sendErrorJson(400, "Habitat Pieces profile must be 1, 2, or 3");
    return;
  }
  context.habitat_pieces_editor_profile_index =
      static_cast<std::uint8_t>(profile_number - 1U);
  cancelHabitatCycle(context);
  resetHabitatPieces(
      context, static_cast<robot::Milliseconds>(millis()));
  clearFault(context);
  sendOkJson("Habitat Pieces profile selected");
}
...
void handleHabitatPiecesConfig() {
  ...
  if (robot::habitatCycleActive(context.habitat_cycle) ||
      ...
      context.habitat_pieces.state !=
          robot::HabitatPiecesState::Fault)) {
    sendErrorJson(409, "stop the Habitat sequence before changing its config");
    return;
  }
  ...
  const std::uint8_t profile_index =
      static_cast<std::uint8_t>(profile_number - 1U);
  ...
  float exit_strafe_duty = next.exit_strafe_duty;
  float approach_forward_duty = next.approach_forward_duty;
  float post_pickup_reverse_duty =
      next.post_pickup_reverse_duty;
  float rear_line_reacquire_duty =
      next.rear_line_reacquire_duty;
  ...
  if (g_server.hasArg("post-count-stop-delay-ms")) {
    if (!argUnsigned("post-count-stop-delay-ms", value,
                     next.post_count_stop_delay_ms, true)) {
      sendErrorJson(400, "malformed post-count-stop-delay-ms");
      return;
    }
    next.post_count_stop_delay_ms = value;
  }
  if (g_server.hasArg("exit-strafe-pulse-ms")) {
    if (!argUnsigned("exit-strafe-pulse-ms", value,
                     next.exit_strafe_pulse_ms, true)) {
      sendErrorJson(400, "malformed exit-strafe-pulse-ms");
      return;
    }
    next.exit_strafe_pulse_ms = value;
  }
  if (g_server.hasArg("exit-strafe-duty") &&
      !argFloat("exit-strafe-duty", exit_strafe_duty,
                next.exit_strafe_duty, true)) {
    sendErrorJson(400, "malformed exit-strafe-duty");
    return;
  }
  next.exit_strafe_duty = exit_strafe_duty;
  if (g_server.hasArg("slide-down-speed")) {
    if (!argUnsigned("slide-down-speed", value,
                     next.slide_down_speed_steps_per_second, true)) {
      sendErrorJson(400, "malformed slide-down-speed");
      return;
    }
  ...
  if (g_server.hasArg("approach-duty") &&
      !argFloat("approach-duty", approach_forward_duty,
                next.approach_forward_duty, true)) {
    sendErrorJson(400, "malformed approach-duty");
    return;
  }
  next.approach_forward_duty = approach_forward_duty;
  if (g_server.hasArg("approach-timeout-ms")) {
    if (!argUnsigned("approach-timeout-ms", value,
                     next.approach_timeout_ms, true)) {
      sendErrorJson(400, "malformed approach-timeout-ms");
      return;
    }
    next.approach_timeout_ms = value;
  }
  if (g_server.hasArg("pre-lift-reverse-duration-ms")) {
    if (!argUnsigned("pre-lift-reverse-duration-ms", value,
                     next.pre_lift_reverse_duration_ms, true)) {
      sendErrorJson(400, "malformed pre-lift-reverse-duration-ms");
      return;
    }
    next.pre_lift_reverse_duration_ms = value;
  }
  if (g_server.hasArg("lift-steps")) {
    if (!argUnsigned("lift-steps", value, next.lift_steps, true)) {
      sendErrorJson(400, "malformed lift-steps");
      return;
    }
    next.lift_steps = value;
  }
  if (g_server.hasArg("lift-speed")) {
    if (!argUnsigned("lift-speed", value,
                     next.lift_speed_steps_per_second, true)) {
      sendErrorJson(400, "malformed lift-speed");
      return;
    }
    next.lift_speed_steps_per_second = value;
  }
  if (g_server.hasArg("lift-timeout-ms")) {
    if (!argUnsigned("lift-timeout-ms", value,
                     next.lift_timeout_ms, true)) {
      sendErrorJson(400, "malformed lift-timeout-ms");
      return;
    }
    next.lift_timeout_ms = value;
  }
  if (g_server.hasArg("lift-start-delay-ms")) {
    if (!argUnsigned("lift-start-delay-ms", value,
                     next.lift_start_delay_ms, true)) {
      sendErrorJson(400, "malformed lift-start-delay-ms");
      return;
    }
    next.lift_start_delay_ms = value;
  }
  if (g_server.hasArg("post-pickup-reverse-duty") &&
      !argFloat("post-pickup-reverse-duty",
                post_pickup_reverse_duty,
  ...
  if (g_server.hasArg("rear-line-duty") &&
      !argFloat("rear-line-duty", rear_line_reacquire_duty,
                next.rear_line_reacquire_duty, true)) {
    sendErrorJson(400, "malformed rear-line-duty");
    return;
  }
  next.rear_line_reacquire_duty = rear_line_reacquire_duty;
  if (g_server.hasArg("rear-line-timeout-ms")) {
    if (!argUnsigned("rear-line-timeout-ms", value,
                     next.rear_line_reacquire_timeout_ms, true)) {
      sendErrorJson(400, "malformed rear-line-timeout-ms");
      return;
    }
    next.rear_line_reacquire_timeout_ms = value;
  }
  ...
  if (!robot::habitatPiecesConfigValid(next,
      ...
      !robot::validateLineFollowerConfig(line_config, hardwareDutyCap())
           .accepted ||
      !autonomousStrafeDutyValid(context,
                                 next.distance_strafe_duty) ||
      !autonomousStrafeDutyValid(context,
                                 next.exit_strafe_duty) ||
      !autonomousStrafeDutyValid(context,
                                 next.rear_line_reacquire_duty) ||
      next.slide_down_speed_steps_per_second >
          g_runtime.stepper->maximumSpeedStepsPerSecond() ||
      next.lift_speed_steps_per_second >
          g_runtime.stepper->maximumSpeedStepsPerSecond() ||
      next.lift_steps > static_cast<std::uint64_t>(
                            g_runtime.stepper->maximumPositionSteps())) {
    sendErrorJson(
        409,
        "Habitat Pieces requires safe duties, thresholds, stepper speeds/steps, and nonzero timeouts no greater than 30000 ms");
    return;
  }

  context.habitat_pieces_profiles[profile_index] = next;
  context.habitat_pieces_editor_profile_index = profile_index;
...
void handleHabitatPlacementProfileSelect() {
  ...
  const robot::HabitatPlacementState state =
      context.habitat_placement.state;
  if (robot::habitatCycleActive(context.habitat_cycle) ||
      (state != robot::HabitatPlacementState::WaitForStart &&
      state != robot::HabitatPlacementState::Complete &&
      state != robot::HabitatPlacementState::Fault)) {
    sendErrorJson(409,
                  "stop Habitat Placement before selecting a profile");
    return;
  }
  robot::Milliseconds profile_number =
      static_cast<robot::Milliseconds>(
  ...
  if (!g_server.hasArg("profile") ||
      ...
      profile_number == 0U ||
      profile_number > robot::kHabitatProfileCount) {
    sendErrorJson(400, "Habitat Placement profile must be 1, 2, or 3");
    return;
  }
  context.habitat_placement_editor_profile_index =
      static_cast<std::uint8_t>(profile_number - 1U);
  ...
  resetHabitatPlacement(
      context, static_cast<robot::Milliseconds>(millis()));
  clearFault(context);
  sendOkJson("Habitat Placement profile selected");
}
...
void handleHabitatPlacementConfig() {
  ...
  robot::HabitatPlacementConfig next =
      context.habitat_placement_profiles[profile_index];
  if (g_server.hasArg("return-line-source")) {
    const String source = g_server.arg("return-line-source");
    if (source == "FRONT" || source == "front") {
      next.return_line_source =
          robot::HabitatPlacementReturnLineSource::Front;
    } else if (source == "REAR" || source == "rear") {
      next.return_line_source =
          robot::HabitatPlacementReturnLineSource::Rear;
    } else {
      sendErrorJson(400, "return-line-source must be FRONT or REAR");
      return;
    }
  }
  ...
  if (!readFloat("reverse-line-duty", next.reverse_line_follow_duty) ||
      ...
      !readMilliseconds(
          "post-cw-strafe-right-ms",
          next.post_clockwise_strafe_right_duration_ms) ||
  ...
  if (!timings_bounded ||
      !robot::habitatPlacementConfigValid(
          ...
          g_runtime.stepper->maximumSpeedStepsPerSecond()) ||
      !habitatPlacementImuStrafeConfigValid(context, next) ||
      !robot::validateLineFollowerConfig(line_config, hardwareDutyCap())
           .accepted) {
    sendErrorJson(
        409,
        "Habitat Placement needs nonzero duties and durations (including pre-CCW right strafe and after-CW reverse/left/right strafe), strafe duties that leave room for IMU yaw correction, search/turn timeouts, positive turn angles, a valid slide speed, and every timing at or below 30000 ms");
    return;
  }

  context.habitat_placement_profiles[profile_index] = next;
  context.habitat_placement_editor_profile_index = profile_index;
...
void handleAutonomousSolarConfig() {
  ...
  if (g_server.hasArg("line-reacquire-timeout-ms")) {
    if (!argUnsigned(
            "line-reacquire-timeout-ms", milliseconds_value,
            next_contact.line_reacquire_strafe_timeout_ms, true)) {
      sendErrorJson(400, "malformed line-reacquire-timeout-ms");
      return;
    }
    next_contact.line_reacquire_strafe_timeout_ms = milliseconds_value;
  }
  ...
  if (!solarThresholdsValid(next_thresholds) ||
      ...
      next_contact.funnel_open_duration_ms >
          kMaxTimedTestDurationMs ||
      next_contact.line_reacquire_strafe_timeout_ms >
          kMaxTimedTestDurationMs) {
    sendErrorJson(409,
                  "solar config requires release <= detect, alpha [0,1), contact/line-reacquire timeouts > 0 and <= 30000 ms, route duties within their caps, and a nonzero signed funnel-open duty/duration within the funnel cap and 30000 ms");
    return;
  }
...
void handleRearLineFollowStart() {
  ...
  stream_warming = g_runtime.rear_link->rearLineStreamWarming();
  if (!g_runtime.rear_link->rearLineAcquisitionRequested()) {
    const bool request_sent = sendStoppedRearCommand(
        *g_runtime.rear_link, context.rear_config, now_ms,
        robot::kOperationalLaserDistanceProfile,
        habitatLaserAcquisitionRequested(context),
        solarIrAcquisitionRequested(context), true);
    stream_warming =
        request_sent && g_runtime.rear_link->rearLineStreamWarming();
  }
  if (!stream_warming && !rearLineStartRequirementsMet(
          *g_runtime.front_left, *g_runtime.front_right,
...
void handleConfigSave() {
  ...
  const robot::HabitatPiecesConfig& pickup =
      context.habitat_pieces_profiles[0U];
  g_runtime.preferences->putFloat(
  ...
  g_runtime.preferences->putFloat(
      "hprevduty", pickup.reverse_duty);
  g_runtime.preferences->putUInt(
      "hprevms", pickup.reverse_duration_ms);
  g_runtime.preferences->putInt(
      "hpdsdir", static_cast<int>(pickup.distance_strafe_direction));
  g_runtime.preferences->putUInt(
      "hpdsthr", pickup.distance_threshold_mm);
  g_runtime.preferences->putUInt(
  ...
  g_runtime.preferences->putUInt(
      "hppostdel", pickup.post_count_stop_delay_ms);
  g_runtime.preferences->putFloat(
      "hpdsexdty", pickup.exit_strafe_duty);
  g_runtime.preferences->putUInt(
      "hpdsexit", pickup.exit_strafe_pulse_ms);
  g_runtime.preferences->putUInt(
      "hpsdnspd", pickup.slide_down_speed_steps_per_second);
  g_runtime.preferences->putUInt(
      "hpsdntmo", pickup.slide_down_timeout_ms);
  g_runtime.preferences->putFloat(
      "hpapduty", pickup.approach_forward_duty);
  g_runtime.preferences->putUInt(
      "hpaptmo", pickup.approach_timeout_ms);
  g_runtime.preferences->putUInt(
      "hpprlms", pickup.pre_lift_reverse_duration_ms);
  g_runtime.preferences->putUInt(
      "hplftstp", pickup.lift_steps);
  g_runtime.preferences->putUInt(
      "hplftspd", pickup.lift_speed_steps_per_second);
  g_runtime.preferences->putUInt(
      "hplfttmo", pickup.lift_timeout_ms);
  g_runtime.preferences->putUInt(
      "hplftdel", pickup.lift_start_delay_ms);
  g_runtime.preferences->putFloat(
      "hpprevd", pickup.post_pickup_reverse_duty);
  g_runtime.preferences->putUInt(
      "hpprevms", pickup.post_pickup_reverse_duration_ms);
  g_runtime.preferences->putFloat(
      "hpreldty", pickup.rear_line_reacquire_duty);
  g_runtime.preferences->putUInt(
      "hprltmo", pickup.rear_line_reacquire_timeout_ms);
  g_runtime.preferences->putUInt(
      "hppkprofv", kHabitatPiecesProfilesPersistenceVersion);
  g_runtime.preferences->putBytes(
      "hppkprof", context.habitat_pieces_profiles.data(),
      sizeof(context.habitat_pieces_profiles));
  const robot::HabitatPlacementConfig& placement =
      context.habitat_placement_profiles[0U];
  ...
  g_runtime.preferences->putUInt(
      "hpcwstrrms",
      placement.post_clockwise_strafe_right_duration_ms);
  ...
  g_runtime.preferences->putUInt(
      "hpprofv", kHabitatPlacementProfilesPersistenceVersion);
  g_runtime.preferences->putBytes(
      "hpplprof", context.habitat_placement_profiles.data(),
      sizeof(context.habitat_placement_profiles));
...
void setupWebHandlers() {
  ...
  g_server.on("/api/autonomous/habitat-pieces/profile", HTTP_ANY,
              handleHabitatPiecesProfileSelect);
  ...
  g_server.on("/api/autonomous/habitat-placement/profile", HTTP_ANY,
              handleHabitatPlacementProfileSelect);
...
void printStatus(const RuntimeContext& context, const RearCommandLink& rear_link,
  ...
  Serial.print(
      context.solar_contact_config.line_reacquire_strafe_start_delay_ms);
  Serial.print(", solar-line-reacquire-timeout-ms=");
  Serial.print(
      context.solar_contact_config.line_reacquire_strafe_timeout_ms);
...
void processCommand(RuntimeContext& context, char* line,
  ...
  if (std::strcmp(token, "habitat") == 0 ||
    ...
    if (command != nullptr && std::strcmp(command, "profile") == 0) {
      robot::Milliseconds profile_number = 0U;
      if (!parseUnsigned(strtok(nullptr, " \t\r\n"), profile_number) ||
          profile_number == 0U ||
          profile_number > robot::kHabitatProfileCount) {
        printRejected("habitat profile <1|2|3>");
        return;
      }
      if (robot::habitatCycleActive(context.habitat_cycle) ||
          (context.habitat_pieces.state !=
           ...
           context.habitat_pieces.state !=
               robot::HabitatPiecesState::Fault)) {
        printRejected("stop the Habitat sequence before selecting a profile");
        return;
      }
      context.habitat_pieces_editor_profile_index =
          static_cast<std::uint8_t>(profile_number - 1U);
      ...
      resetHabitatPieces(context, now_ms);
      clearFault(context);
      printOk("Habitat Pieces profile selected");
      return;
    }
    if (command != nullptr && std::strcmp(command, "config") == 0) {
      ...
      next.exit_strafe_duty = exit_strafe_duty;
      next.exit_strafe_pulse_ms = exit_strafe_pulse_ms;
      robot::LineFollowerConfig line_config = context.config;
      ...
      if (robot::habitatCycleActive(context.habitat_cycle) ||
          (context.habitat_pieces.state !=
           ...
           context.habitat_pieces.state !=
               robot::HabitatPiecesState::Fault)) {
        printRejected("stop the Habitat sequence before changing config");
        return;
      }
      context.habitat_pieces_profiles[
          context.habitat_pieces_editor_profile_index] = next;
      context.habitat_pieces_config = next;
      cancelHabitatCycle(context);
      resetHabitatPieces(context, now_ms);
      clearFault(context);
    ...
    printRejected("habitat start|stop|status|profile|config");
    return;
  }
  ...
  if (std::strcmp(token, "lf") == 0 ||
    ...
    if (std::strcmp(command, "start") == 0) {
      ...
      bool rear_line_stream_warming =
          rear_line_command && rear_link.rearLineStreamWarming();
      if (rear_line_command &&
          !rear_link.rearLineAcquisitionRequested()) {
        const bool request_sent = sendStoppedRearCommand(
            rear_link, context.rear_config, now_ms,
            robot::kOperationalLaserDistanceProfile,
            habitatLaserAcquisitionRequested(context),
            solarIrAcquisitionRequested(context), true);
        rear_line_stream_warming =
            request_sent && rear_link.rearLineStreamWarming();
      }
      const bool requirements_met =
          rear_line_command
              ? rear_line_stream_warming ||
                    rearLineStartRequirementsMet(
                        front_left, front_right, rear_link, now_ms, context)
...
void loadPreferences(Preferences& preferences, RuntimeContext& context,
  ...
  context.habitat_pieces_config.lift_steps =
      preferences.getUInt(
          "hplftstp", context.habitat_pieces_config.lift_steps);
  context.habitat_pieces_config.lift_speed_steps_per_second =
      preferences.getUInt(
          "hplftspd",
          context.habitat_pieces_config
  ...
  context.habitat_pieces_config.lift_timeout_ms =
      preferences.getUInt(
          "hplfttmo", context.habitat_pieces_config.lift_timeout_ms);
  context.habitat_pieces_config.lift_start_delay_ms =
      preferences.getUInt(
          "hplftdel",
          context.habitat_pieces_config.lift_start_delay_ms);
  ...
  context.habitat_pieces_config.rear_line_reacquire_timeout_ms =
      preferences.getUInt(
          "hprltmo",
          context.habitat_pieces_config
              .rear_line_reacquire_timeout_ms);
  ...
  placement.return_line_source =
      robot::HabitatPlacementReturnLineSource::Front;
  context.habitat_placement_profiles.fill(placement);
  context.habitat_placement_profiles[2U].return_line_source =
      robot::HabitatPlacementReturnLineSource::Rear;
  const std::size_t placement_profiles_size =
      sizeof(context.habitat_placement_profiles);
  const std::uint32_t placement_profiles_version =
      preferences.getUInt("hpprofv", 0U);
  if (placement_profiles_version ==
          kHabitatPlacementProfilesPersistenceVersion &&
      preferences.getBytesLength("hpplprof") ==
          placement_profiles_size) {
    const std::size_t loaded_profile_bytes = preferences.getBytes(
        "hpplprof", context.habitat_placement_profiles.data(),
        placement_profiles_size);
    if (loaded_profile_bytes != placement_profiles_size) {
      context.habitat_placement_profiles.fill(placement);
      context.habitat_placement_profiles[2U].return_line_source =
          robot::HabitatPlacementReturnLineSource::Rear;
    }
  } else if (placement_profiles_version == 1U) {
    constexpr std::size_t kLegacyProfilesSize =
        kLegacyHabitatPlacementProfileSize *
        robot::kHabitatProfileCount;
    if (preferences.getBytesLength("hpplprof") ==
        kLegacyProfilesSize) {
      std::array<std::uint8_t, kLegacyProfilesSize> legacy_profiles{};
      if (preferences.getBytes(
              "hpplprof", legacy_profiles.data(),
              legacy_profiles.size()) == legacy_profiles.size()) {
        for (std::size_t index = 0U;
             index < robot::kHabitatProfileCount; ++index) {
          robot::HabitatPlacementConfig migrated{};
          std::uint8_t* const migrated_bytes =
              reinterpret_cast<std::uint8_t*>(&migrated);
          std::memcpy(
              static_cast<void*>(migrated_bytes),
              legacy_profiles.data() +
                  index * kLegacyHabitatPlacementProfileSize,
              kLegacyHabitatPlacementProfileSize);
          migrated.return_line_source =
              index + 1U == robot::kHabitatProfileCount
                  ? robot::HabitatPlacementReturnLineSource::Rear
                  : robot::HabitatPlacementReturnLineSource::Front;
          context.habitat_placement_profiles[index] = migrated;
        }
      }
    }
  }
...
void motionControlTask(void* parameters) {
  ...
  // RuntimeContext grows with each autonomous route. Keep this task-owned
  // state out of the fixed-size FreeRTOS stack so telemetry handling cannot
  // exhaust the motion task's stack as routes are added.
  ...
  static DigitalActiveHighLimitSwitch final_competition_start_switch{
      robot::esp2::kPins.final_competition_start_switch};
  ...
  final_competition_start_switch.initialize();
  ...
  g_runtime = {&context, &line_sensor_reader, &habitat_piece_limit,
               &peg_finder_funnel_limit, &front_left_motor,
               &front_right_motor, &rear_link, &claws, &stepper,
               &imu_acquisition, &preferences};
  ...
  for (;;) {
    ...
    const robot::Milliseconds now_ms =
        static_cast<robot::Milliseconds>(millis());
    final_competition_start_switch.update();
    serviceFinalCompetitionStartSwitch(
        context, final_competition_start_switch, front_left_motor,
    ...
    const bool rear_line_stream_requested =
        rearLineAcquisitionRequested(context);
    if (rear_line_stream_requested !=
        rear_link.rearLineAcquisitionRequested()) {
      (void)sendStoppedRearCommand(
          rear_link, context.rear_config, now_ms,
          robot::kOperationalLaserDistanceProfile,
          habitatLaserAcquisitionRequested(context),
          solarIrAcquisitionRequested(context),
          rear_line_stream_requested);
    }
    const bool rear_line_stream_warming =
        rear_line_stream_requested && rear_link.rearLineStreamWarming() &&
        elapsedSince(now_ms, rear_link.rearLineRequestStartedAtMs()) <=
            remoteStatusTimeoutMs(context.rear_config);

    if (rear_line_stream_warming) {
      context.requested_command = robot::disabledFourWheelCommand();
      context.last_commanded_wheels = robot::disabledFourWheelCommand();
      ...
      (void)sendStoppedRearCommand(
          ...
          habitatLaserAcquisitionRequested(context),
          solarIrAcquisitionRequested(context), true);
    } else if (robot::robotTestModeIsSensorOnly(mode)) {
      disableActuators(context, front_left_motor, front_right_motor, rear_link,
                       now_ms);
      ...
      runHabitatPieces(context, line_sensor_reader, front_left_motor,
                       ...
                       habitat_piece_limit, stepper, now_ms);
