...
void test_valid_rear_command_is_accepted() {
  ...
  command.back_left_command_milli = 123;
  command.back_right_command_milli = -456;
  command.sender_timestamp_ms = 100U;
  command.timeout_ms = 250U;
  command.laser_acquisition_enabled = true;
  command.ir_acquisition_enabled = true;
  command.rear_line_acquisition_enabled = true;
  ...
  TEST_ASSERT_TRUE(receiver.laserAcquisitionEnabled(120U));
  TEST_ASSERT_TRUE(receiver.irAcquisitionEnabled(120U));
  TEST_ASSERT_TRUE(receiver.rearLineAcquisitionEnabled(120U));
}
...
void test_rear_command_retains_laser_profile_when_stale() {
  ...
  TEST_ASSERT_TRUE(receiver.laserAcquisitionEnabled(150U));
  TEST_ASSERT_TRUE(receiver.irAcquisitionEnabled(150U));
  TEST_ASSERT_FALSE(receiver.laserAcquisitionEnabled(151U));
  TEST_ASSERT_FALSE(receiver.irAcquisitionEnabled(151U));
  TEST_ASSERT_FALSE(receiver.rearLineAcquisitionEnabled(151U));
}

void test_command_transmission_policy_repeats_transitions_then_refreshes() {
  robot::CommandTransmissionState state{};
  const robot::Milliseconds refresh_period_ms =
      robot::commandRefreshPeriodMs(250U);
  TEST_ASSERT_EQUAL_UINT32(125U, refresh_period_ms);
  TEST_ASSERT_TRUE(robot::commandTransmissionDue(
      state, true, false, 100U, refresh_period_ms));
  robot::noteCommandTransmitted(state, true, 100U);
  TEST_ASSERT_EQUAL_UINT8(1U, state.transition_repeats_remaining);

  TEST_ASSERT_TRUE(robot::commandTransmissionDue(
      state, false, false, 110U, refresh_period_ms));
  robot::noteCommandTransmitted(state, false, 110U);
  TEST_ASSERT_EQUAL_UINT8(0U, state.transition_repeats_remaining);
  TEST_ASSERT_FALSE(robot::commandTransmissionDue(
      state, false, false, 224U, refresh_period_ms));
  TEST_ASSERT_TRUE(robot::commandTransmissionDue(
      state, false, false, 235U, refresh_period_ms));
  TEST_ASSERT_TRUE(robot::commandTransmissionDue(
      state, false, true, 111U, refresh_period_ms));
}

void test_final_competition_switch_requires_low_then_rising_edge_to_start() {
  robot::FinalCompetitionStartSwitchState state{};

  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::FinalCompetitionStartSwitchAction::None),
      static_cast<std::uint8_t>(robot::updateFinalCompetitionStartSwitch(
          state, true, false)));
  TEST_ASSERT_FALSE(state.start_armed);
  ...
  TEST_ASSERT_EQUAL_UINT8(
      ...
      static_cast<std::uint8_t>(robot::updateFinalCompetitionStartSwitch(
          state, false, false)));
  TEST_ASSERT_TRUE(state.start_armed);

  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::FinalCompetitionStartSwitchAction::Start),
      static_cast<std::uint8_t>(robot::updateFinalCompetitionStartSwitch(
          state, true, false)));
  TEST_ASSERT_FALSE(state.start_armed);
...
void test_final_competition_switch_low_requests_emergency_stop_and_rearms() {
  robot::FinalCompetitionStartSwitchState state{};
  (void)robot::updateFinalCompetitionStartSwitch(state, false, false);
  (void)robot::updateFinalCompetitionStartSwitch(state, true, false);

  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::FinalCompetitionStartSwitchAction::EmergencyStop),
      static_cast<std::uint8_t>(robot::updateFinalCompetitionStartSwitch(
          state, false, true)));
  TEST_ASSERT_TRUE(state.start_armed);
...
void test_final_competition_mode_parses_and_allows_distributed_motion() {
  robot::RobotTestMode mode{};

  TEST_ASSERT_TRUE(robot::parseRobotTestMode("final-competition", mode));
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::RobotTestMode::FinalCompetition),
      static_cast<std::uint8_t>(mode));
  TEST_ASSERT_EQUAL_STRING("FINAL_COMPETITION",
                           robot::robotTestModeName(mode));
  TEST_ASSERT_TRUE(robot::robotTestModeAllowsMotion(mode));
  TEST_ASSERT_TRUE(robot::robotTestModeRequiresRearLink(mode));
  TEST_ASSERT_FALSE(robot::robotTestModeIsSensorOnly(mode));
...
void test_imu_turn_to_heading_keeps_the_absolute_target() {
  robot::ImuTurnControllerState state{};
  const robot::ImuTurnConfig config = imuTurnConfig();

  TEST_ASSERT_TRUE(robot::startImuTurnToHeading(
      state, 47.0F, 12.5F, config, 0.5F, 25U));
  assertNear(47.0F, state.start_heading_deg, 0.0001F);
  assertNear(12.5F, state.target_heading_deg, 0.0001F);
  assertNear(-34.5F, state.relative_angle_deg, 0.0001F);

  (void)robot::updateImuTurn(
      state, 30.0F, -3.0F, config, 0.5F, 35U);
  assertNear(12.5F, state.target_heading_deg, 0.0001F);

  robot::resetImuTurnController(state);
  TEST_ASSERT_TRUE(robot::startImuTurnToHeading(
      state, 12.5F, 12.5F, config, 0.5F, 40U));
  const robot::ImuTurnUpdate update = robot::updateImuTurn(
      state, 12.5F, 0.0F, config, 0.5F, 40U);
  TEST_ASSERT_EQUAL_UINT8(
...
void test_time_trial_does_not_handoff_before_solar_is_complete() {
...
void test_final_competition_runs_all_modes_in_order() {
  robot::FinalCompetitionAutonomy autonomy{};
  robot::FinalCompetitionUpdate update =
      robot::startFinalCompetitionAutonomy(autonomy, 10U);
  TEST_ASSERT_TRUE(update.should_start_solar);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  inputs.solar_complete = true;
  update = robot::updateFinalCompetitionAutonomy(autonomy, inputs, 20U);
  TEST_ASSERT_TRUE(update.should_start_habitat);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  inputs = {};
  inputs.habitat_complete = true;
  update = robot::updateFinalCompetitionAutonomy(autonomy, inputs, 30U);
  TEST_ASSERT_TRUE(update.should_start_tower);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  inputs = {};
  inputs.tower_complete = true;
  update = robot::updateFinalCompetitionAutonomy(autonomy, inputs, 40U);
  TEST_ASSERT_TRUE(update.should_start_peg_finder);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  inputs = {};
  inputs.peg_finder_complete = true;
  update = robot::updateFinalCompetitionAutonomy(autonomy, inputs, 50U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::FinalCompetitionState::Complete),
      static_cast<std::uint8_t>(update.state));

  robot::resetFinalCompetitionAutonomy(autonomy, 60U);
  (void)robot::startFinalCompetitionAutonomy(autonomy, 61U);
  inputs = {};
  inputs.solar_fault = true;
  update = robot::updateFinalCompetitionAutonomy(autonomy, inputs, 62U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::FinalCompetitionState::Fault),
      static_cast<std::uint8_t>(update.state));
}
...
void test_peg_finder_config_requires_safe_duties_angle_and_timings() {
  ...
  config = pegFinderConfig();
  config.shake_duty = 0.1F;
  TEST_ASSERT_FALSE(robot::pegFinderConfigValid(config, 0.5F, 0.4F));
  config.shake_left_duration_ms = 5U;
  config.shake_right_duration_ms = 7U;
  TEST_ASSERT_TRUE(robot::pegFinderConfigValid(config, 0.5F, 0.4F));
...
void test_peg_finder_selects_one_open_claw_in_configured_order_then_reverses_funnel() {
  robot::PegFinderConfig config = pegFinderConfig();
  ...
  TEST_ASSERT_TRUE(update.should_close_claw_1);
  TEST_ASSERT_FALSE(update.should_open_claw_2);
  TEST_ASSERT_TRUE(update.should_close_claw_2);
  TEST_ASSERT_FALSE(update.should_close_claw_3);
  ...
  TEST_ASSERT_FALSE(update.should_close_claw_1);
  TEST_ASSERT_FALSE(update.should_open_claw_2);
  TEST_ASSERT_TRUE(update.should_close_claw_2);
  TEST_ASSERT_TRUE(update.should_close_claw_3);
  ...
  TEST_ASSERT_TRUE(update.should_close_claw_1);
  TEST_ASSERT_FALSE(update.should_close_claw_2);
  TEST_ASSERT_TRUE(update.should_close_claw_3);
...
void test_peg_finder_shakes_left_and_right_after_each_claw_drop() {
  robot::PegFinderConfig config = pegFinderConfig();
  config.claw_open_interval_ms = 10U;
  config.shake_duty = 0.1F;
  config.shake_left_duration_ms = 5U;
  config.shake_right_duration_ms = 7U;
  config.post_shake_delay_ms = 11U;
  robot::PegFinderAutonomy autonomy{};
  autonomy.state = robot::PegFinderState::PostClaw1OpenDelay;
  autonomy.state_entered_at_ms = 100U;

  robot::PegFinderUpdate update =
      updatePegFinderForTest(autonomy, config, 109U);
  TEST_ASSERT_FALSE(update.should_shake_left);
  update = updatePegFinderForTest(autonomy, config, 110U);
  TEST_ASSERT_TRUE(update.should_shake_left);
  TEST_ASSERT_FALSE(update.should_shake_right);
  update = updatePegFinderForTest(autonomy, config, 115U);
  TEST_ASSERT_FALSE(update.should_shake_left);
  TEST_ASSERT_TRUE(update.should_shake_right);
  update = updatePegFinderForTest(autonomy, config, 122U);
  TEST_ASSERT_FALSE(update.should_shake_right);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::PostShakeAfterClaw1Delay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_FALSE(update.should_open_claw_2);
  update = updatePegFinderForTest(autonomy, config, 132U);
  TEST_ASSERT_FALSE(update.should_open_claw_2);
  ...
  update = updatePegFinderForTest(autonomy, config, 134U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::PostClaw2OpenDelay),
      static_cast<std::uint8_t>(update.state));
  update = updatePegFinderForTest(autonomy, config, 144U);
  TEST_ASSERT_TRUE(update.should_shake_left);
  update = updatePegFinderForTest(autonomy, config, 149U);
  TEST_ASSERT_TRUE(update.should_shake_right);
  update = updatePegFinderForTest(autonomy, config, 156U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::PostShakeAfterClaw2Delay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_FALSE(update.should_open_claw_3);
  update = updatePegFinderForTest(autonomy, config, 166U);
  TEST_ASSERT_FALSE(update.should_open_claw_3);
  update = updatePegFinderForTest(autonomy, config, 167U);
  TEST_ASSERT_TRUE(update.should_open_claw_3);

  update = updatePegFinderForTest(autonomy, config, 168U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::PostClawsOpenDelay),
      static_cast<std::uint8_t>(update.state));
  update = updatePegFinderForTest(autonomy, config, 185U);
  TEST_ASSERT_FALSE(update.should_shake_left);
  update = updatePegFinderForTest(autonomy, config, 186U);
  TEST_ASSERT_TRUE(update.should_shake_left);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::ShakeLeftAfterClaw3),
      static_cast<std::uint8_t>(update.state));
  update = updatePegFinderForTest(autonomy, config, 191U);
  TEST_ASSERT_TRUE(update.should_shake_right);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::ShakeRightAfterClaw3),
      static_cast<std::uint8_t>(update.state));
  update = updatePegFinderForTest(autonomy, config, 198U);
  TEST_ASSERT_FALSE(update.should_shake_right);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::PegFinderState::PostShakeAfterClaw3Delay),
      static_cast<std::uint8_t>(update.state));
  update = updatePegFinderForTest(autonomy, config, 208U);
  TEST_ASSERT_FALSE(update.should_run_funnel_reverse);
  update = updatePegFinderForTest(autonomy, config, 209U);
  TEST_ASSERT_TRUE(update.should_run_funnel_reverse);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::PegFinderState::FunnelReverse),
      static_cast<std::uint8_t>(update.state));
...
void test_tower_pieces_config_requires_duties_and_timings() {
  ...
  TEST_ASSERT_EQUAL_UINT8(
      ...
      static_cast<std::uint8_t>(defaults.shimmy_initial_direction));
  TEST_ASSERT_EQUAL_UINT32(0U, defaults.pre_shimmy_delay_ms);
  TEST_ASSERT_EQUAL_UINT32(0U, defaults.post_shimmy_delay_ms);
  ...
  config.shimmy_initial_direction =
      static_cast<robot::TowerPiecesShimmyInitialDirection>(0);
  TEST_ASSERT_FALSE(robot::towerPiecesConfigValid(config, 0.5F, 200000U));
  config = towerPiecesConfig();
  config.shimmy_initial_direction =
      robot::TowerPiecesShimmyInitialDirection::Right;
  config.shimmy_right_duration_ms = 1U;
  TEST_ASSERT_FALSE(robot::towerPiecesConfigValid(config, 0.5F, 200000U));
  config = towerPiecesConfig();
  config.shimmy_initial_direction =
      robot::TowerPiecesShimmyInitialDirection::Left;
  config.shimmy_left_duration_ms = 1U;
  TEST_ASSERT_FALSE(robot::towerPiecesConfigValid(config, 0.5F, 200000U));
...
void test_tower_pieces_ignores_held_high_until_low_rearms_sensor() {
  robot::TowerPiecesAutonomy autonomy{};
  robot::TowerPiecesConfig config = towerPiecesConfig();
  ...
  robot::TowerPiecesUpdate update = updateTowerPiecesForTest(
      autonomy, true, false, false, config, 199U);
  TEST_ASSERT_TRUE(autonomy.side_line_ignore_active);
  TEST_ASSERT_FALSE(update.side_line_detection_accepted);
  TEST_ASSERT_EQUAL_UINT8(0U, update.side_line_count);

  update = updateTowerPiecesForTest(
      autonomy, true, false, false, config, 200U);
  TEST_ASSERT_FALSE(autonomy.side_line_ignore_active);
  TEST_ASSERT_FALSE(update.side_line_rising_edge);
  TEST_ASSERT_EQUAL_UINT8(0U, update.side_line_count);

  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 201U);
  TEST_ASSERT_FALSE(update.side_line_armed);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 221U);
  TEST_ASSERT_TRUE(update.side_line_armed);
  update = updateTowerPiecesForTest(
      autonomy, true, false, false, config, 222U);
...
void test_tower_pieces_runs_full_sequence_in_order() {
  ...
  update = updateTowerPiecesForTest(autonomy, true, false, false,
                                            config, 1600U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::TowerPiecesState::PreShimmyDelay),
      static_cast<std::uint8_t>(update.state));
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::TowerPiecesState::PostShimmyDelay),
      static_cast<std::uint8_t>(update.state));
...
void test_tower_pieces_shimmy_direction_half_pulse_and_delays() {
  robot::TowerPiecesConfig config = towerPiecesConfig();
  config.shimmy_initial_direction =
      robot::TowerPiecesShimmyInitialDirection::Left;
  config.pre_shimmy_delay_ms = 20U;
  config.post_shimmy_delay_ms = 30U;
  robot::TowerPiecesAutonomy autonomy{};
  autonomy.state = robot::TowerPiecesState::ReverseTimed;
  autonomy.state_entered_at_ms = 100U;

  robot::TowerPiecesUpdate update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 400U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::TowerPiecesState::PreShimmyDelay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_FALSE(update.should_shimmy_left);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 419U);
  TEST_ASSERT_FALSE(update.should_shimmy_left);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 420U);
  TEST_ASSERT_TRUE(update.should_shimmy_left);
  TEST_ASSERT_TRUE(autonomy.first_shimmy_pulse);
  TEST_ASSERT_EQUAL_STRING(
      "LEFT", robot::towerPiecesShimmyInitialDirectionName(
                  config.shimmy_initial_direction));

  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 479U);
  TEST_ASSERT_TRUE(update.should_shimmy_left);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 480U);
  TEST_ASSERT_TRUE(update.should_shimmy_right);
  TEST_ASSERT_FALSE(autonomy.first_shimmy_pulse);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 559U);
  TEST_ASSERT_TRUE(update.should_shimmy_right);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 560U);
  TEST_ASSERT_TRUE(update.should_shimmy_left);

  update = updateTowerPiecesForTest(
      autonomy, false, true, false, config, 561U);
  TEST_ASSERT_TRUE(update.back_line_detected);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::TowerPiecesState::PostShimmyDelay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_FALSE(update.should_drive_final_reverse);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 590U);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::TowerPiecesState::PostShimmyDelay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_TRUE(update.back_line_detected);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 591U);
  TEST_ASSERT_TRUE(update.should_drive_final_reverse);

  config.shimmy_initial_direction =
      robot::TowerPiecesShimmyInitialDirection::Right;
  autonomy = {};
  autonomy.state = robot::TowerPiecesState::PreShimmyDelay;
  autonomy.state_entered_at_ms = 1000U;
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 1020U);
  TEST_ASSERT_TRUE(update.should_shimmy_right);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 1059U);
  TEST_ASSERT_TRUE(update.should_shimmy_right);
  update = updateTowerPiecesForTest(
      autonomy, false, false, false, config, 1060U);
  TEST_ASSERT_TRUE(update.should_shimmy_left);
}
...
void test_tower_pieces_shimmy_hands_off_to_tail_and_can_skip_reverse() {
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::TowerPiecesState::PostShimmyDelay),
      static_cast<std::uint8_t>(update.state));
  TEST_ASSERT_FALSE(update.should_line_follow);
  TEST_ASSERT_FALSE(update.should_drive_final_reverse);
  update = updateTowerPiecesForTest(
...
void test_solar_post_contact_forward_stops_if_front_line_is_already_detected() {
  const robot::SolarPanelContactConfig config = solarContactConfig();
  ...
  assertSolarState(
      robot::SolarPanelAutonomyState::ForwardLineFollowAfterFrontDetection,
      update.next_state);
  TEST_ASSERT_TRUE(update.transitioned);
}
...
void test_solar_post_contact_delays_are_independently_adjustable() {
  ...
  assertSolarState(
      robot::SolarPanelAutonomyState::WaitBeforeStrafeLeftToFrontLine,
      update.next_state);
  TEST_ASSERT_TRUE(update.transitioned);
  ...
  assertSolarState(robot::SolarPanelAutonomyState::StrafeLeftToFrontLine,
                   update.next_state);
  TEST_ASSERT_TRUE(update.transitioned);
}

void test_solar_first_timeout_without_front_contact_continues_forward() {
  const robot::SolarPanelContactConfig config = solarContactConfig();
...
void test_solar_second_strafe_timeout_continues_forward_without_retry() {
  const robot::SolarPanelContactConfig config = solarContactConfig();
  ...
  update = robot::updateSolarPanelContactSequence(
      update.next_state, true, false, 0U, config);
  assertSolarState(
      robot::SolarPanelAutonomyState::MoveForwardAfterSolarContact,
      update.next_state);
  TEST_ASSERT_FALSE(update.transitioned);
...
void test_solar_contact_drives_forward_then_strafes_until_front_line() {
  const robot::SolarPanelContactConfig config = solarContactConfig();
  ...
  update = robot::updateSolarPanelContactSequence(
      robot::SolarPanelAutonomyState::StrafeLeftToFrontLine, true, true,
      true, config.line_reacquire_strafe_timeout_ms, config);
  assertSolarState(
      robot::SolarPanelAutonomyState::ForwardLineFollowAfterFrontDetection,
                   update.next_state);
  TEST_ASSERT_TRUE(update.transitioned);
  ...
  update = robot::updateSolarPanelContactSequence(
      update.next_state, true, true, false,
      config.front_line_follow_duration_ms - 1U, config);
  assertSolarState(
      robot::SolarPanelAutonomyState::ForwardLineFollowAfterFrontDetection,
      update.next_state);
  TEST_ASSERT_FALSE(update.transitioned);
  ...
  update = robot::updateSolarPanelContactSequence(
      update.next_state, true, true, false,
      config.front_line_follow_duration_ms, config);
  assertSolarState(robot::SolarPanelAutonomyState::FrontLineFollowComplete,
                   update.next_state);
  TEST_ASSERT_TRUE(update.transitioned);
}

void test_solar_hook_opens_only_after_front_line_detection() {
  const robot::SolarPanelContactConfig config = solarContactConfig();
  ...
  assertSolarState(robot::SolarPanelAutonomyState::StrafeLeftToFrontLine,
                   update.next_state);
  TEST_ASSERT_FALSE(
      robot::shouldOpenSolarHookAfterLineDetection(update));

  update = robot::updateSolarPanelContactSequence(
      robot::SolarPanelAutonomyState::StrafeLeftToFrontLine, true, true,
      false, config.line_reacquire_strafe_timeout_ms - 1U, config);
  TEST_ASSERT_FALSE(update.transitioned);
  TEST_ASSERT_FALSE(
      robot::shouldOpenSolarHookAfterLineDetection(update));
...
void test_telemetry_json_contains_required_fields_and_booleans() {
  ...
  snapshot.ir_acquisition_enabled = true;
  snapshot.esp1.ir_acquisition_enabled = true;
  ...
  snapshot.solar_line_reacquire_strafe_timeout_ms = 1200U;
  ...
  snapshot.habitat_pieces_side_line_ignore_after_start_ms = 500U;
  ...
  snapshot.habitat_pieces_distance_threshold_mm = 320U;
  snapshot.habitat_pieces_distance_zone_target_count = 3U;
  snapshot.habitat_pieces_distance_strafe_duty = 0.16F;
  snapshot.habitat_pieces_distance_count_ignore_ms = 1000U;
  snapshot.habitat_pieces_distance_count_ignore_remaining_ms = 100U;
  snapshot.habitat_pieces_distance_strafe_timeout_ms = 4000U;
  snapshot.habitat_pieces_distance_strafe_elapsed_ms = 900U;
  ...
  snapshot.habitat_pieces_distance_exit_pulse_count = 4U;
  ...
  snapshot.habitat_pieces_lss3_configured = true;
  snapshot.habitat_pieces_lss3_data_fresh = true;
  snapshot.habitat_pieces_lss2_detection_armed = true;
  snapshot.habitat_pieces_lss2_black = true;
  snapshot.habitat_pieces_lss3_black = false;
  snapshot.habitat_pieces_lss2_latched = true;
  snapshot.habitat_pieces_lss3_latched = false;
  snapshot.habitat_pieces_should_stop = false;
  snapshot.habitat_pieces_target_reached = false;
  snapshot.habitat_pieces_line_following = false;
  snapshot.habitat_pieces_side_line_aligning = false;
  snapshot.habitat_pieces_rotating_clockwise = false;
  snapshot.habitat_pieces_rotating_counter_clockwise = false;
  snapshot.habitat_pieces_reversing = false;
  snapshot.habitat_pieces_distance_strafing = true;
  snapshot.habitat_pieces_post_count_waiting = false;
  snapshot.habitat_pieces_exit_strafe_pulsing = false;
  snapshot.habitat_pieces_exit_distance_checking = false;
  snapshot.habitat_pieces_distance_measurement_available = true;
  snapshot.habitat_pieces_distance_substituted_no_target = true;
  snapshot.habitat_pieces_distance_sample_new = true;
  snapshot.habitat_pieces_distance_count_ignore_active = true;
  snapshot.habitat_pieces_distance_zone_active = false;
  snapshot.habitat_pieces_distance_zone_entered = false;
  snapshot.habitat_pieces_distance_exit_above_threshold = false;
  ...
  snapshot.habitat_placement_config
      .pre_counter_clockwise_strafe_right_duty = 0.19F;
  snapshot.habitat_placement_config
      .pre_counter_clockwise_strafe_right_duration_ms = 350U;
  ...
  snapshot.tower_pieces_side_line_ignore_after_start_ms = 450U;
  ...
  snapshot.tower_pieces_shimmy_initial_direction =
      robot::TowerPiecesShimmyInitialDirection::Left;
  snapshot.tower_pieces_pre_shimmy_delay_ms = 175U;
  ...
  snapshot.tower_pieces_post_shimmy_delay_ms = 275U;
  ...
  snapshot.tower_pieces_side_line_ignore_active = true;
  ...
  snapshot.peg_finder_shake_duty = 0.1F;
  snapshot.peg_finder_shake_left_duration_ms = 40U;
  snapshot.peg_finder_shake_right_duration_ms = 50U;
  snapshot.peg_finder_post_shake_delay_ms = 60U;
  ...
  snapshot.peg_finder_shaking_left = true;
  snapshot.peg_finder_shaking_right = true;
  ...
  snapshot.final_competition_state =
      robot::FinalCompetitionState::HabitatCycle;
  snapshot.final_competition_time_in_state_ms = 123U;
  ...
  char output[robot::kTelemetryJsonBufferSize]{};
  TEST_ASSERT_TRUE(
      robot::writeTelemetryJson(snapshot, output, sizeof(output), false));
  TEST_ASSERT_TRUE(std::strlen(output) + 512U < sizeof(output));
  ...
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"initial_heading_deg\":12.50000"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"counter_clockwise_target_heading_deg\":-77.50000"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"initial_heading_turn_timeout_ms\":600"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output,
      "\"pre_counter_clockwise_strafe_right_duration_ms\":350"));
  TEST_ASSERT_NOT_NULL(std::strstr(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"ir_acquisition_enabled\":true"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"forward_pid_duration_ms\":750"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"forward_pid_duty\":0.21000"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"post_contact_forward_start_delay_ms\":125"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"line_reacquire_strafe_start_delay_ms\":250"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"line_reacquire_strafe_timeout_ms\":1200"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"side_line_ignore_after_start_ms\":450"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"side_line_ignore_active\":true"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shimmy_initial_direction\":\"LEFT\""));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"pre_shimmy_delay_ms\":175"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"post_shimmy_delay_ms\":275"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shake_duty\":0.10000"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shake_left_duration_ms\":40"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shake_right_duration_ms\":50"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"post_shake_delay_ms\":60"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shaking_left\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"shaking_right\":true"));
  TEST_ASSERT_NOT_NULL(std::strstr(output, "\"time_trial\""));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"state\":\"SOLAR_TO_TOWER_STRAFE_RIGHT\""));
  TEST_ASSERT_NOT_NULL(std::strstr(output, "\"final_competition\""));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"state\":\"HABITAT_CYCLE\""));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"state\":\"DISTANCE_STRAFING\""));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"line_follow_duty\":0.12000"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"side_line_ignore_after_start_ms\":500"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_strafe_direction\":\"LEFT\""));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_threshold_mm\":320"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_zone_target_count\":3"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_strafe_duty\":0.16000"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_count_ignore_ms\":1000"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"distance_count_ignore_remaining_ms\":100"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_exit_pulse_count\":4"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_mm\":65536"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_zone_count\":2"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_strafing\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_measurement_available\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_substituted_no_target\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"distance_zone_active\":false"));
  TEST_ASSERT_NOT_NULL(std::strstr(
      output, "\"distance_count_ignore_active\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"lss2_detection_armed\":true"));
  TEST_ASSERT_NOT_NULL(
  ...
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"lss3_configured\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"lss3_data_fresh\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"lss2_latched\":true"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"lss3_latched\":false"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"side_line_aligning\":false"));
  TEST_ASSERT_NOT_NULL(
      std::strstr(output, "\"rotating_counter_clockwise\":false"));
  TEST_ASSERT_NOT_NULL(
...
void test_esp1_status_packet_round_trips() {
  ...
  report.ir_acquisition_enabled = true;
  ...
  TEST_ASSERT_EQUAL_UINT16(41U, packet.header.payload_size);
  ...
  TEST_ASSERT_TRUE(decoded.ir_acquisition_enabled);
...
void test_esp1_operational_and_diagnostic_packets_split_fields() {
  robot::Esp1StatusReport report{};
  report.uptime_ms = 3210U;
  report.mode = robot::RobotTestMode::AutonomousSolarPanel;
  report.fault_active = true;
  report.fault_code = robot::FaultCode::CommunicationStale;
  report.back_left_applied_command_milli = 101;
  report.back_right_applied_command_milli = -202;
  report.funnel_applied_command_milli = 303;
  report.funnel_configured = true;
  report.solar_panel_limit_switches_configured = true;
  report.solar_limit_back_right_high = true;
  report.solar_hook_configured = true;
  report.solar_hook_output_enabled = true;
  report.solar_hook_commanded_angle_deg = 120;
  report.ir_acquisition_enabled = true;
  report.ir_beacon_detected = true;
  report.ir_selected_frequency_hz = 10000U;
  report.ir_selected_amplitude = 456U;
  report.side_line_sensor_configured = true;
  report.side_line_sensor_high = true;
  report.ir_switch_raw_high = false;
  report.ir_switch_debounced_high = false;
  report.ir_adc_average = 1800U;
  report.ir_adc_min = 1200U;
  report.ir_adc_max = 2200U;
  report.ir_amplitude_pp = 1000U;
  report.ir_adc_latest_sample = 1700U;
  report.ir_1khz_amplitude = 25U;
  report.ir_10khz_amplitude = 456U;
  report.ir_active_threshold = 120U;
  report.ir_consecutive_detection_count = 3U;
  report.ir_adc_sample_rate_hz = 50000U;

  const robot::UartPacket operational =
      robot::makeEsp1OperationalStatusPacket(report, 10U);
  TEST_ASSERT_EQUAL_UINT16(robot::kEsp1OperationalStatusPayloadSize,
                           operational.header.payload_size);
  robot::Esp1StatusReport decoded{};
  decoded.ir_adc_average = 77U;
  TEST_ASSERT_TRUE(robot::decodeEsp1StatusPacket(operational, decoded));
  TEST_ASSERT_EQUAL_UINT32(3210U, decoded.uptime_ms);
  TEST_ASSERT_EQUAL_INT16(101, decoded.back_left_applied_command_milli);
  TEST_ASSERT_EQUAL_INT16(-202, decoded.back_right_applied_command_milli);
  TEST_ASSERT_EQUAL_INT16(303, decoded.funnel_applied_command_milli);
  TEST_ASSERT_TRUE(decoded.funnel_configured);
  TEST_ASSERT_TRUE(decoded.solar_panel_limit_switches_configured);
  TEST_ASSERT_TRUE(decoded.solar_limit_back_right_high);
  TEST_ASSERT_FALSE(decoded.solar_limit_front_right_high);
  TEST_ASSERT_TRUE(decoded.solar_hook_configured);
  TEST_ASSERT_TRUE(decoded.solar_hook_output_enabled);
  TEST_ASSERT_EQUAL_INT16(120, decoded.solar_hook_commanded_angle_deg);
  TEST_ASSERT_TRUE(decoded.ir_acquisition_enabled);
  TEST_ASSERT_TRUE(decoded.ir_beacon_detected);
  TEST_ASSERT_EQUAL_UINT16(10000U, decoded.ir_selected_frequency_hz);
  TEST_ASSERT_EQUAL_UINT16(456U, decoded.ir_selected_amplitude);
  TEST_ASSERT_TRUE(decoded.side_line_sensor_configured);
  TEST_ASSERT_TRUE(decoded.side_line_sensor_high);
  TEST_ASSERT_EQUAL_UINT16(77U, decoded.ir_adc_average);

  const robot::UartPacket diagnostics =
      robot::makeEsp1DiagnosticsPacket(report, 11U);
  TEST_ASSERT_EQUAL_UINT16(robot::kEsp1DiagnosticsPayloadSize,
                           diagnostics.header.payload_size);
  TEST_ASSERT_TRUE(
      robot::decodeEsp1DiagnosticsPacket(diagnostics, decoded));
  TEST_ASSERT_EQUAL_UINT32(3210U, decoded.uptime_ms);
  TEST_ASSERT_FALSE(decoded.ir_switch_raw_high);
  TEST_ASSERT_FALSE(decoded.ir_switch_debounced_high);
  TEST_ASSERT_EQUAL_UINT16(1800U, decoded.ir_adc_average);
  TEST_ASSERT_EQUAL_UINT16(1200U, decoded.ir_adc_min);
  TEST_ASSERT_EQUAL_UINT16(2200U, decoded.ir_adc_max);
  TEST_ASSERT_EQUAL_UINT16(1000U, decoded.ir_amplitude_pp);
  TEST_ASSERT_EQUAL_UINT16(1700U, decoded.ir_adc_latest_sample);
  TEST_ASSERT_EQUAL_UINT16(25U, decoded.ir_1khz_amplitude);
  TEST_ASSERT_EQUAL_UINT16(456U, decoded.ir_10khz_amplitude);
  TEST_ASSERT_EQUAL_UINT16(120U, decoded.ir_active_threshold);
  TEST_ASSERT_EQUAL_UINT8(3U, decoded.ir_consecutive_detection_count);
  TEST_ASSERT_EQUAL_UINT32(50000U, decoded.ir_adc_sample_rate_hz);
}
...
void test_habitat_pieces_defaults_to_requested_duty_but_stays_locked() {
  ...
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  config.lift_speed_steps_per_second = 800U;
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  config.lift_timeout_ms = 3000U;
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  ...
  config.distance_count_ignore_ms = 350U;
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  ...
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  config.distance_strafe_timeout_ms = 500U;
  config.exit_strafe_pulse_ms = 501U;
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
  config.exit_strafe_pulse_ms = 450U;
  TEST_ASSERT_FALSE(robot::habitatPiecesConfigValid(config, 1.0F, 30000U));
...
void test_habitat_pieces_ignores_both_side_sensors_until_start_gate() {
  robot::HabitatPiecesConfig config{};
  config.side_line_ignore_after_start_ms = 100U;
  config.run_timeout_ms = 1000U;
  config.slide_down_timeout_ms = 1000U;
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, true, true, 199U);
  TEST_ASSERT_TRUE(update.should_line_follow);
  TEST_ASSERT_FALSE(update.should_stop);
  TEST_ASSERT_FALSE(update.lss2_detection_armed);
  TEST_ASSERT_TRUE(update.lss2_black);
  TEST_ASSERT_TRUE(update.lss3_black);
  TEST_ASSERT_FALSE(update.lss2_latched);
  TEST_ASSERT_FALSE(update.lss3_latched);
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::HabitatPiecesState::LineFollowing),
      static_cast<std::uint8_t>(update.state));
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, true, true, 200U);
  TEST_ASSERT_FALSE(update.should_line_follow);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.should_reverse);
  TEST_ASSERT_TRUE(update.lss2_detection_armed);
  TEST_ASSERT_TRUE(update.lss2_latched);
  TEST_ASSERT_TRUE(update.lss3_latched);
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
...
void test_habitat_pieces_lss2_stops_rotates_clockwise_until_lss3() {
  robot::HabitatPiecesConfig config{};
  config.side_line_ignore_after_start_ms = 100U;
  config.run_timeout_ms = 1000U;
  config.slide_down_timeout_ms = 1000U;
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 200U);
  TEST_ASSERT_TRUE(update.should_line_follow);
  TEST_ASSERT_TRUE(update.lss2_detection_armed);
  TEST_ASSERT_FALSE(update.target_reached);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, true, false, 250U);
  TEST_ASSERT_FALSE(update.should_line_follow);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.should_reverse);
  TEST_ASSERT_FALSE(update.should_align_side_lines);
  TEST_ASSERT_FALSE(update.should_rotate_clockwise);
  TEST_ASSERT_FALSE(update.should_rotate_counter_clockwise);
  TEST_ASSERT_TRUE(update.lss2_latched);
  TEST_ASSERT_FALSE(update.lss3_latched);
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_EQUAL_UINT8(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 260U);
  TEST_ASSERT_TRUE(update.should_align_side_lines);
  TEST_ASSERT_TRUE(update.should_rotate_clockwise);
  TEST_ASSERT_FALSE(update.should_rotate_counter_clockwise);
  TEST_ASSERT_FALSE(update.should_reverse);
  TEST_ASSERT_TRUE(update.lss2_latched);
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, true, 270U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.should_align_side_lines);
  TEST_ASSERT_FALSE(update.should_rotate_clockwise);
  TEST_ASSERT_FALSE(update.should_rotate_counter_clockwise);
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesState::ClosePusherBeforeDistanceStrafe),
      static_cast<std::uint8_t>(update.state));

  robot::HabitatPiecesMechanismInputs mechanisms{};
  mechanisms.pusher_closed_commanded = true;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 371U, {}, mechanisms);
  TEST_ASSERT_FALSE(update.should_close_pusher);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesState::DistanceStrafing),
...
void test_habitat_pieces_lss3_stops_rotates_counter_clockwise_until_lss2() {
  robot::HabitatPiecesConfig config{};
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, true, 110U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.should_align_side_lines);
  TEST_ASSERT_FALSE(update.should_reverse);
  TEST_ASSERT_FALSE(update.should_rotate_clockwise);
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesStopReason::Lss3Detected),
      static_cast<std::uint8_t>(update.stop_reason));
  TEST_ASSERT_EQUAL_STRING(
      "LSS3_DETECTED",
      robot::habitatPiecesStopReasonName(update.stop_reason));
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 120U);
  TEST_ASSERT_TRUE(update.should_align_side_lines);
  TEST_ASSERT_FALSE(update.should_rotate_clockwise);
  TEST_ASSERT_TRUE(update.should_rotate_counter_clockwise);
  TEST_ASSERT_FALSE(update.should_reverse);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, true, false, 130U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.should_align_side_lines);
  TEST_ASSERT_TRUE(update.lss2_latched);
  TEST_ASSERT_TRUE(update.lss3_latched);
...
void test_habitat_pieces_times_out_while_waiting_for_a_side_line() {
  robot::HabitatPiecesConfig config{};
  config.side_line_ignore_after_start_ms = 10U;
...
void test_habitat_pieces_times_out_during_side_line_alignment() {
  robot::HabitatPiecesConfig config{};
  config.side_line_ignore_after_start_ms = 10U;
  config.run_timeout_ms = 50U;
  config.slide_down_timeout_ms = 1000U;
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, true, false, 110U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesState::SideLineAligning),
...
void test_habitat_pieces_distance_strafe_counts_distinct_zone_entries() {
  robot::HabitatPiecesConfig config{};
  config.distance_strafe_direction =
      robot::HabitatPiecesStrafeDirection::Right;
  config.distance_threshold_mm = 300U;
  config.distance_zone_target_count = 2U;
  config.distance_strafe_duty = 0.2F;
  config.distance_strafe_timeout_ms = 500U;
  config.post_count_stop_delay_ms = 50U;
  config.exit_strafe_pulse_ms = 100U;
  ...
  sample.distance_mm = 250U;
  sample.measurement_sequence = 10U;
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 110U, sample);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  TEST_ASSERT_TRUE(update.distance_sample_new);
  TEST_ASSERT_TRUE(update.distance_zone_entered);
  TEST_ASSERT_TRUE(update.distance_zone_active);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_zone_count);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 120U, sample);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  TEST_ASSERT_FALSE(update.distance_sample_new);
  TEST_ASSERT_FALSE(update.distance_zone_entered);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_zone_count);

  sample.available = false;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 130U, sample);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  ...
  sample.available = true;
  sample.measurement_sequence = 11U;
  sample.distance_mm = 200U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 140U, sample);
  TEST_ASSERT_FALSE(update.distance_zone_entered);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_zone_count);

  sample.measurement_sequence = 12U;
  sample.distance_mm = 301U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 150U, sample);
  TEST_ASSERT_FALSE(update.distance_zone_active);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_zone_count);

  sample.measurement_sequence = 13U;
  sample.distance_mm = 300U;
  update = robot::updateHabitatPiecesAutonomy(
  ...
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_TRUE(update.should_wait_after_distance_count);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesStopReason::DistanceZoneCountReached),
      static_cast<std::uint8_t>(update.stop_reason));
}

void test_habitat_pieces_distance_count_ignore_requires_a_new_zone_entry() {
  robot::HabitatPiecesConfig config{};
  config.distance_strafe_direction =
      robot::HabitatPiecesStrafeDirection::Right;
  config.distance_threshold_mm = 300U;
  config.distance_zone_target_count = 2U;
  config.distance_strafe_duty = 0.2F;
  config.distance_count_ignore_ms = 30U;
  config.distance_strafe_timeout_ms = 500U;
  ...
  sample.available = true;
  sample.distance_mm = 250U;
  sample.measurement_sequence = 1U;
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 110U, sample);
  TEST_ASSERT_TRUE(update.distance_count_ignore_active);
  TEST_ASSERT_TRUE(update.distance_zone_active);
  TEST_ASSERT_FALSE(update.distance_zone_entered);
  TEST_ASSERT_EQUAL_UINT16(0U, update.distance_zone_count);

  sample.measurement_sequence = 2U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 131U, sample);
  TEST_ASSERT_FALSE(update.distance_count_ignore_active);
  TEST_ASSERT_TRUE(update.distance_zone_active);
  TEST_ASSERT_FALSE(update.distance_zone_entered);
  TEST_ASSERT_EQUAL_UINT16(0U, update.distance_zone_count);

  sample.measurement_sequence = 3U;
  sample.distance_mm = 350U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 140U, sample);
  TEST_ASSERT_FALSE(update.distance_zone_active);
  TEST_ASSERT_EQUAL_UINT16(0U, update.distance_zone_count);

  sample.measurement_sequence = 4U;
  sample.distance_mm = 250U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 150U, sample);
  TEST_ASSERT_TRUE(update.distance_zone_entered);
...
void test_habitat_pieces_distance_strafe_timeout_runs_exit_sequence() {
  robot::HabitatPiecesConfig config{};
  config.distance_strafe_direction =
      robot::HabitatPiecesStrafeDirection::Left;
  config.distance_threshold_mm = 250U;
  config.distance_zone_target_count = 1U;
  config.distance_strafe_duty = 0.15F;
  config.distance_strafe_timeout_ms = 50U;
  config.post_count_stop_delay_ms = 10U;
  config.exit_strafe_pulse_ms = 10U;
  config.slide_down_timeout_ms = 1000U;
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 159U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_wait_after_distance_count);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 160U);
  TEST_ASSERT_FALSE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_exit_strafe_pulse);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::HabitatPiecesState::ExitStrafePulse),
      static_cast<std::uint8_t>(update.state));
}

void test_habitat_pieces_no_target_substitution_is_always_above_threshold() {
  TEST_ASSERT_TRUE(
      robot::kHabitatPiecesNoTargetDistanceMm > UINT16_MAX);

  robot::HabitatPiecesConfig config{};
  config.distance_threshold_mm = UINT16_MAX;
  config.distance_zone_target_count = 2U;
  config.distance_strafe_timeout_ms = 500U;
  config.post_count_stop_delay_ms = 50U;
  config.exit_strafe_pulse_ms = 100U;
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 110U, sample);
  TEST_ASSERT_TRUE(update.distance_measurement_available);
  ...
  sample.measurement_sequence = 14U;
  sample.distance_mm = UINT16_MAX;
  sample.substituted_no_target = false;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 150U, sample);
...
void test_habitat_pieces_distance_strafe_ignores_pre_step_sample() {
  robot::HabitatPiecesConfig config{};
  config.reverse_duration_ms = 100U;
  config.distance_strafe_direction =
      robot::HabitatPiecesStrafeDirection::Right;
  ...
  robot::HabitatPiecesAutonomy autonomy{};
  autonomy.state = robot::HabitatPiecesState::Reversing;
  autonomy.stop_reason =
      robot::HabitatPiecesStopReason::Lss2Detected;
  autonomy.state_entered_at_ms = 100U;
  ...
  sample.available = true;
  sample.distance_mm = 250U;
  sample.measurement_sequence = 20U;
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 200U, sample);
  TEST_ASSERT_TRUE(update.should_close_pusher);
  TEST_ASSERT_FALSE(update.should_distance_strafe);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT16(0U, update.distance_zone_count);
  ...
  mechanisms.pusher_closed_commanded = true;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 201U, sample, mechanisms);
  TEST_ASSERT_FALSE(update.should_close_pusher);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 202U, sample, mechanisms);
  TEST_ASSERT_TRUE(update.should_distance_strafe);
  TEST_ASSERT_FALSE(update.distance_sample_new);
  TEST_ASSERT_EQUAL_UINT16(0U, update.distance_zone_count);
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 400U, sample, mechanisms);
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_TRUE(update.should_wait_after_distance_count);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_zone_count);
  TEST_ASSERT_EQUAL_UINT8(
...
void test_habitat_pieces_repeats_stopped_exit_pulses_until_above_threshold() {
  robot::HabitatPiecesConfig config{};
  config.distance_threshold_mm = 300U;
  config.distance_zone_target_count = 2U;
  config.distance_strafe_timeout_ms = 1000U;
  config.post_count_stop_delay_ms = 50U;
  config.exit_strafe_pulse_ms = 100U;

  robot::HabitatPiecesAutonomy autonomy{};
  autonomy.state = robot::HabitatPiecesState::PostCountStopDelay;
  autonomy.stop_reason =
      robot::HabitatPiecesStopReason::DistanceZoneCountReached;
  autonomy.state_entered_at_ms = 100U;
  autonomy.distance_strafe_started_at_ms = 100U;
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 149U, sample);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_wait_after_distance_count);
  TEST_ASSERT_EQUAL_UINT32(49U, autonomy.post_count_stop_elapsed_ms);
  ...
  TEST_ASSERT_TRUE(update.should_exit_strafe_pulse);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT16(1U, update.distance_exit_pulse_count);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 249U, sample);
  TEST_ASSERT_TRUE(update.should_exit_strafe_pulse);
  TEST_ASSERT_EQUAL_UINT32(99U, autonomy.exit_strafe_pulse_elapsed_ms);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 250U, sample);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_check_exit_distance);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 259U, sample);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_check_exit_distance);
  TEST_ASSERT_FALSE(update.distance_sample_new);
  ...
  sample.measurement_sequence = 11U;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 260U, sample);
  TEST_ASSERT_FALSE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_exit_strafe_pulse);
  TEST_ASSERT_TRUE(update.distance_sample_new);
  TEST_ASSERT_FALSE(update.distance_exit_above_threshold);
  TEST_ASSERT_EQUAL_UINT16(2U, update.distance_exit_pulse_count);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 360U, sample);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_check_exit_distance);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  sample.substituted_no_target = true;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 370U, sample);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_FALSE(update.target_reached);
  TEST_ASSERT_TRUE(update.distance_exit_above_threshold);
  TEST_ASSERT_FALSE(update.distance_zone_active);
  TEST_ASSERT_TRUE(update.distance_substituted_no_target);
  TEST_ASSERT_EQUAL_UINT8(
  ...
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
          robot::HabitatPiecesStopReason::DistanceExitReached),
      static_cast<std::uint8_t>(update.stop_reason));
  TEST_ASSERT_EQUAL_STRING(
      "DISTANCE_EXIT_REACHED",
      robot::habitatPiecesStopReasonName(update.stop_reason));
}
...
void test_habitat_pieces_pickup_tail_multitasks_lift_and_hands_off() {
  ...
  robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 109U, sample, mechanisms);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_lower_slide);
  ...
  mechanisms.bottom_limit_active = true;
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 110U, sample, mechanisms);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(robot::HabitatPiecesState::ApproachPiece),
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 120U, sample, mechanisms);
...
void test_habitat_pieces_approach_limit_timeout_continues_pickup_tail() {
  robot::HabitatPiecesConfig config{};
  config.approach_timeout_ms = 50U;
  config.pre_lift_reverse_duration_ms = 20U;
  config.lift_timeout_ms = 500U;
  config.lift_start_delay_ms = 10U;
  config.post_pickup_reverse_duration_ms = 30U;
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 169U, {}, mechanisms);
  TEST_ASSERT_FALSE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_drive_back_before_lift);
  TEST_ASSERT_FALSE(update.should_start_lift);

  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 170U, {}, mechanisms);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_start_lift);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 180U, {}, mechanisms);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
  ...
  update = robot::updateHabitatPiecesAutonomy(
      autonomy, config, false, false, 181U, {}, mechanisms);
  TEST_ASSERT_FALSE(update.should_stop);
  TEST_ASSERT_TRUE(update.should_drive_back_after_pickup);
}

void test_habitat_pieces_exit_timeout_advances_to_slide_wait() {
  robot::HabitatPiecesConfig config{};
  config.distance_strafe_timeout_ms = 200U;
  config.post_count_stop_delay_ms = 50U;
  config.exit_strafe_pulse_ms = 100U;

  robot::HabitatPiecesAutonomy autonomy{};
  autonomy.state = robot::HabitatPiecesState::ExitDistanceCheck;
  autonomy.stop_reason =
      robot::HabitatPiecesStopReason::DistanceZoneCountReached;
  autonomy.state_entered_at_ms = 250U;
  autonomy.distance_strafe_started_at_ms = 100U;

  const robot::HabitatPiecesUpdate update =
      robot::updateHabitatPiecesAutonomy(
          autonomy, config, false, false, 300U);
  TEST_ASSERT_TRUE(update.should_stop);
  TEST_ASSERT_TRUE(update.transitioned);
  TEST_ASSERT_FALSE(autonomy.timed_out);
  TEST_ASSERT_EQUAL_UINT8(
...
robot::HabitatPlacementConfig validHabitatPlacementConfig() {
  ...
  config.initial_heading_turn_timeout_ms = 100U;
  config.pre_counter_clockwise_strafe_right_duty = 0.2F;
  config.pre_counter_clockwise_strafe_right_duration_ms = 10U;
...
void test_habitat_imu_phase_classification_covers_turns_and_strafes() {
  TEST_ASSERT_TRUE(robot::habitatPiecesStateRequiresImuStrafe(
      robot::HabitatPiecesState::DistanceStrafing));
  TEST_ASSERT_TRUE(robot::habitatPiecesStateRequiresImuStrafe(
      robot::HabitatPiecesState::ExitStrafePulse));
  TEST_ASSERT_TRUE(robot::habitatPiecesStateRequiresImuStrafe(
      robot::HabitatPiecesState::RearLineReacquire));
  TEST_ASSERT_FALSE(robot::habitatPiecesStateRequiresImuStrafe(
      robot::HabitatPiecesState::SideLineAligning));

  TEST_ASSERT_TRUE(robot::habitatPlacementStateRequiresImuTurn(
      robot::HabitatPlacementState::TurnToInitialHeading));
  TEST_ASSERT_TRUE(robot::habitatPlacementStateRequiresImuTurn(
      robot::HabitatPlacementState::TurnCounterClockwise));
  TEST_ASSERT_TRUE(robot::habitatPlacementStateRequiresImuTurn(
      robot::HabitatPlacementState::TurnClockwise));
  TEST_ASSERT_FALSE(robot::habitatPlacementStateRequiresImuTurn(
      robot::HabitatPlacementState::ReverseRetreat));

  TEST_ASSERT_TRUE(robot::habitatPlacementStateRequiresImuStrafe(
      robot::HabitatPlacementState::StrafeRightBeforeCounterClockwise));
  TEST_ASSERT_TRUE(robot::habitatPlacementStateRequiresImuStrafe(
  ...
  TEST_ASSERT_FALSE(robot::habitatPlacementStateRequiresImuStrafe(
      robot::HabitatPlacementState::TurnClockwise));
}
...
void test_habitat_placement_runs_requested_sequence() {
  ...
  TEST_ASSERT_TRUE(robot::startHabitatPlacementAutonomy(
      autonomy, 12.5F, -90.0F, 0U));
  TEST_ASSERT_TRUE(autonomy.initial_heading_captured);
  TEST_ASSERT_FLOAT_WITHIN(0.0001F, 12.5F,
                           autonomy.initial_heading_deg);
  TEST_ASSERT_FLOAT_WITHIN(
      0.0001F, -77.5F,
  ...
  update = robot::updateHabitatPlacementAutonomy(autonomy, inputs, config,
                                                 11U);
  TEST_ASSERT_TRUE(update.should_turn_to_initial_heading);
  inputs.initial_heading_turn_complete = true;
  update = robot::updateHabitatPlacementAutonomy(autonomy, inputs, config,
                                                 12U);
  TEST_ASSERT_TRUE(
...
void test_habitat_placement_initial_heading_turn_times_out_stopped() {
  const robot::HabitatPlacementConfig config =
      validHabitatPlacementConfig();
  robot::HabitatPlacementAutonomy autonomy{};
  TEST_ASSERT_TRUE(robot::startHabitatPlacementAutonomy(
      autonomy, 30.0F, -90.0F, 0U));

  robot::HabitatPlacementInputs inputs{};
  inputs.lss1_black = true;
  (void)robot::updateHabitatPlacementAutonomy(
      autonomy, inputs, config, 1U);
  inputs = {};
  robot::HabitatPlacementUpdate update =
      robot::updateHabitatPlacementAutonomy(
          autonomy, inputs, config, 11U);
  TEST_ASSERT_TRUE(update.should_turn_to_initial_heading);

  update = robot::updateHabitatPlacementAutonomy(
      autonomy, inputs, config, 111U);
  TEST_ASSERT_TRUE(update.faulted);
  TEST_ASSERT_TRUE(update.should_stop_drive);
  TEST_ASSERT_EQUAL_UINT8(
      static_cast<std::uint8_t>(
...
int main() {
  ...
  RUN_TEST(
      test_command_transmission_policy_repeats_transitions_then_refreshes);
  RUN_TEST(
      test_final_competition_switch_requires_low_then_rising_edge_to_start);
  RUN_TEST(
      test_final_competition_switch_low_requests_emergency_stop_and_rearms);
  ...
  RUN_TEST(
      test_final_competition_mode_parses_and_allows_distributed_motion);
  ...
  RUN_TEST(test_imu_turn_to_heading_keeps_the_absolute_target);
  ...
  RUN_TEST(test_peg_finder_shakes_left_and_right_after_each_claw_drop);
  ...
  RUN_TEST(
      test_tower_pieces_ignores_held_high_until_low_rearms_sensor);
  ...
  RUN_TEST(
      test_solar_first_timeout_without_front_contact_continues_forward);
  ...
  RUN_TEST(
      test_solar_second_strafe_timeout_continues_forward_without_retry);
  ...
  RUN_TEST(
      test_solar_contact_drives_forward_then_strafes_until_front_line);
  RUN_TEST(test_solar_hook_opens_only_after_front_line_detection);
  RUN_TEST(
  ...
  RUN_TEST(test_esp1_operational_and_diagnostic_packets_split_fields);
  RUN_TEST(
  ...
  RUN_TEST(
      test_habitat_pieces_ignores_both_side_sensors_until_start_gate);
  RUN_TEST(
      test_habitat_pieces_lss2_stops_rotates_clockwise_until_lss3);
  RUN_TEST(
      test_habitat_pieces_lss3_stops_rotates_counter_clockwise_until_lss2);
  RUN_TEST(
  ...
  RUN_TEST(
      test_habitat_pieces_distance_count_ignore_requires_a_new_zone_entry);
  RUN_TEST(
      test_habitat_pieces_distance_strafe_timeout_runs_exit_sequence);
  RUN_TEST(
      test_habitat_pieces_no_target_substitution_is_always_above_threshold);
  RUN_TEST(
  ...
  RUN_TEST(
      test_habitat_pieces_approach_limit_timeout_continues_pickup_tail);
  RUN_TEST(
      test_habitat_pieces_exit_timeout_advances_to_slide_wait);
  RUN_TEST(test_habitat_placement_mode_parses_and_allows_motion);
  RUN_TEST(
      test_habitat_imu_phase_classification_covers_turns_and_strafes);
